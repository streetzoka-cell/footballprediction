...
const injectStyles = () => {
  ...
  s.textContent = `
    ...
    .mc-skel{background:linear-gradient(90deg,var(--bg-surface) 25%,var(--bg-card) 50%,var(--bg-surface) 75%);background-size:200% 100%;animation:mc_shimmer 1.5s ease-in-out infinite;border-radius:6px}
    ...
    .mc-card:hover{border-color:rgba(255,255,255,.08);transform:translateY(-1px);box-shadow:0 8px 24px rgba(0,0,0,.15)}
    .mc-card.mc-interactive{cursor:pointer}
    .mc-card.mc-interactive:hover{transform:translateY(-2px);box-shadow:0 12px 32px rgba(0,0,0,.2)}

    .mc-header{display:flex;align-items:center;justify-content:space-between;padding:10px 16px 0;gap:8px;flex-wrap:wrap}
    .mc-league{display:inline-flex;align-items:center;gap:6px;font-size:.7rem;font-weight:600;color:var(--text-muted)}
    ...
    .mc-team-name{font-size:.84rem;font-weight:600;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.2}
    ...
    .mc-score-sep{color:var(--text-muted);font-size:.8rem;font-weight:700}
    .mc-score-label{font-size:.6rem;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.04em}

    .mc-time-display{font-family:var(--font-display);font-size:.95rem;font-weight:700;color:var(--text-muted);text-align:center;padding:6px 0}

    .mc-status-badge{display:inline-flex;align-items:center;gap:4px;font-size:.64rem;font-weight:700;padding:2px 8px;border-radius:5px;letter-spacing:.02em}
    .mc-status-badge.live{color:#ef4444;background:rgba(239,68,68,.1)}
    .mc-status-badge.upcoming{color:var(--text-muted);background:rgba(255,255,255,.04)}
    .mc-status-badge.finished{color:var(--accent);background:rgba(0,230,118,.08)}
    ...
    .mc-minute{font-family:var(--font-display);font-size:.68rem;font-weight:700;color:#ef4444}

    .mc-pred-row{display:flex;align-items:center;gap:5px;font-size:.64rem;color:var(--text-muted);font-weight:500}
    .mc-exact-tag{color:var(--accent);font-weight:700;letter-spacing:.03em}
    ...
    .mc-prob-track{flex:1;height:5px;border-radius:3px;background:rgba(255,255,255,.04);overflow:hidden}
    .mc-prob-fill{height:100%;border-radius:3px;transition:width .9s cubic-bezier(.22,1,.36,1)}
    .mc-prob-fill.home{background:var(--accent)}
    .mc-prob-fill.draw{background:var(--gold)}
    .mc-prob-fill.away{background:#60a5fa}
    .mc-prob-value{width:34px;font-size:.68rem;font-weight:700;text-align:left;font-family:var(--font-display);flex-shrink:0}
    ...
    .mc-odds-chip:hover{border-color:rgba(0,230,118,.25);background:rgba(0,230,118,.04);transform:translateY(-1px)}
    .mc-odds-chip-label{font-size:.6rem;font-weight:600;color:var(--text-muted);text-transform:uppercase;letter-spacing:.04em}
    .mc-odds-chip-value{font-family:var(--font-display);font-size:.92rem;font-weight:800;color:var(--text-primary)}

    .mc-meta{padding:0 16px 12px;display:flex;gap:16px;font-size:.66rem;color:var(--text-muted);font-weight:500}
...
const getConfidence = (h, d, a) => {
  ...
  if (m >= 55) return { label: 'High', color: '#00e676' };
  if (m >= 40) return { label: 'Medium', color: '#f5c542' };
  return { label: 'Low', color: '#ef4444' };
};

/* ═══════════════════════════════════════════════════════════════
   TEAM BADGE — handles logo or color fallback
   ═══════════════════════════════════════════════════════════════ */
const TeamBadge = ({ logo, name, color, abbr }) => {
  const fallback = color || '#444';
  ...
  return (
    <div className="mc-team-badge" style={{ background: fallback }}>
      {logo
        ...
        : <span className="abbr">{initials}</span>
      }
    </div>
  );
};
...
const ScoreDisplay = ({ match, predicted, actual, isLive, isFinished, isScheduled, timeStr, goalFlash, scoreKey }) => {
  if (isScheduled) {
    return <div className="mc-time-display">{timeStr || '--:--'}</div>;
  }
  ...
  return (
    <div className={`mc-score-area ${isFlash ? 'mc-goal-flash' : ''}`}>
      <div className={`mc-score-row ${scoreKey === 'changed' ? 'mc-score-pop' : ''}`}>
        <div
          className="mc-score-box"
          style={{
            borderColor: hOk ? 'var(--accent)' : isPredOnly ? 'rgba(0,230,118,.25)' : 'var(--border)',
            color: isLive ? '#ef4444' : isFinished ? 'var(--text-primary)' : isPredOnly ? 'var(--accent)' : 'var(--text-muted)',
            boxShadow: hOk ? '0 0 10px rgba(0,230,118,.2)' : 'none',
          }}
        >{h}</div>
        ...
        <div
          className="mc-score-box"
          style={{
            borderColor: aOk ? 'var(--accent)' : isPredOnly ? 'rgba(0,230,118,.25)' : 'var(--border)',
            color: isLive ? '#ef4444' : isFinished ? 'var(--text-primary)' : isPredOnly ? 'var(--accent)' : 'var(--text-muted)',
            boxShadow: aOk ? '0 0 10px rgba(0,230,118,.2)' : 'none',
          }}
        >{a}</div>
      </div>
      {predicted && (
...
const ProbBar = ({ label, value, type, delay = 0 }) => {
  ...
  return (
    <div className="mc-prob-row">
      <span className="mc-prob-label" style={{ color: isHigh ? 'var(--text-primary)' : 'var(--text-muted)', fontWeight: isHigh ? 700 : 600 }}>{label}</span>
      <div className="mc-prob-track">
        <div className={`mc-prob-fill ${type}`} style={{ width: `${w}%` }} />
      </div>
      <span className="mc-prob-value" style={{ color: isHigh ? 'var(--text-primary)' : 'var(--text-secondary)' }}>{value}%</span>
    </div>
  );
};
...
export default function MatchCard({
  ...
  const lc = match.league?.color || 'var(--accent)';
  ...
  const predicted = match.predictedHomeScore != null
    ? { home: match.predictedHomeScore, away: match.predictedAwayScore }
    : undefined;
  const actual = (finished && match.actualHomeScore != null)
    ? { home: match.actualHomeScore, away: match.actualAwayScore }
    ...
    return (
      <div className={`mc-card mc-interactive ${borderClass} ${goalFlash ? 'mc-goal-flash' : ''}`} onClick={handleClick} style={{ animationDelay: `${index * 30}ms` }}>
        ...
        <div className="mc-header" style={{ paddingBottom: 6 }}>
          ...
          {!live && !finished && <span style={{ fontSize: '.76rem', color: 'var(--text-muted)', fontWeight: 600 }}>{timeStr}</span>}
        </div>
    ...
    >
      ...
      {kickOff && (
        <div style={{ position: 'absolute', top: 8, right: 12, zIndex: 2 }}>
          <span className="mc-ko-badge" style={{ fontSize: '.58rem', fontWeight: 800, color: '#00e676', background: 'rgba(0,230,118,.12)', padding: '2px 8px', borderRadius: 5, letterSpacing: '.05em' }}>
            KICK OFF
          </span>
        </div>
      )}
      ...
      <div className="mc-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          ...
          {!live && !finished && <span style={{ fontSize: '.68rem', color: 'var(--text-muted)', fontWeight: 500 }}>{dateStr} · {timeStr}</span>}
      ...
      {/* Meta row (referee for finished, etc.) */}
