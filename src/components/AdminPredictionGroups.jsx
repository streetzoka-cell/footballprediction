...
/* ★ LOCKED to pipeline contract §3 — no defensive chains */
...
const pickMatchId = (p) => String(p.matchId || '');
...
/* ★ React #31 FIX: quality_summary is an OBJECT { PURE: 4, STRONG: 5 } — never render raw */
...
export default function AdminPredictionGroups({ embedded = false, date: dateProp = null }) {
  ...
  const isFamilyHidden = (famKey) => (exclusions.familyExclusions || []).includes(famKey);
          ...
          return (
            <div key={famKey} className="mb-24">
              <div className="flex-center gap-12 mb-12 flex-wrap">
                ...
                {isFamilyHidden(famKey) && <span className="badge flex-center gap-4"><EyeOff size={10} /> hidden</span>}
