...
import { ROUTES, STUDIO_ROUTES, buildMatchRoute } from '../utils/routes';
import { applySmartMinute } from '../engine/matchEngine';
import ThemeSwitcher from './ThemeSwitcher';

// ★ SINGLE SOURCE OF TRUTH — zero hardcoded route strings in this file.
// If a path changes, it changes in utils/routes.js and every consumer follows.
const ADMIN_PATH = ROUTES.ADMIN;
...
const LINKS = [
  ...
  { to: ROUTES.FIXTURES, label: 'Fixtures', icon: Activity },
  { to: ROUTES.PREDICTIONS, label: 'Predictions', icon: Target, badge: 'NEW' },
  { to: ROUTES.LEADERBOARD, label: 'Ranks', icon: Trophy },
  { to: ROUTES.HIGHLIGHTS, label: 'News', icon: Newspaper },
  { to: ROUTES.LIVESTREAM, label: 'Stream', icon: Tv, isLive: true },
  { to: STUDIO_ROUTES.HOME, label: 'Studio', icon: Sparkles },
  { to: ROUTES.BASKETBALL, label: 'Hoops', icon: Gamepad2 },
];
const MOBILE_NAV = [
  { to: ROUTES.HOME, label: 'Home', icon: Home },
  { to: ROUTES.FIXTURES, label: 'Live', icon: Activity },
  { to: ROUTES.PREDICTIONS, label: 'Predict', icon: Target },
  { to: ROUTES.LEADERBOARD, label: 'Compete', icon: Trophy },
  { to: ROUTES.PROFILE, label: 'Profile', icon: User },
];
const SYSTEM_LINKS = [
  { title: 'Creator', icon: Sparkles, links: [['Studio', STUDIO_ROUTES.HOME], ['Master Games', ROUTES.MASTERGAMES]] },
  { title: 'Company', icon: Building, links: [['About', ROUTES.ABOUT], ['Contact', ROUTES.CONTACT], ['Careers', ROUTES.CAREERS], ['Partners', ROUTES.PARTNERS], ['Advertise', ROUTES.ADVERTISE], ['Team', ROUTES.TEAM]] },
  // ★ NEW: Developers / Football Knowledge / Changelog / Status were real routes
  // with ZERO internal links → crawl orphans. Now linked (also feeds SEO).
  { title: 'Support', icon: LifeBuoy, links: [['Help Center', ROUTES.HELP], ['FAQ', ROUTES.FAQ], ['Football Knowledge', ROUTES.FOOTBALL_KNOWLEDGE], ['Developers', ROUTES.DEVELOPERS]] },
  { title: 'Legal', icon: Scale, links: [['Privacy Policy', ROUTES.PRIVACY], ['Terms of Service', ROUTES.TERMS], ['Changelog', ROUTES.CHANGELOG], ['Status', ROUTES.STATUS]] },
];
    ...
    <>
      ...
      <header className={`nav-top-bar ${scrolled ? 'scrolled' : ''}`}>
        <div className="nav-top-grid">
          <div className="nav-top-left"><Link to={ROUTES.HOME} className="flex-center gap-8" aria-label="ZOKASCORE Home"><img src={APP_LOGO} alt="ZokaScore" width="32" height="32" className="rounded-8" /><span className="font-extrabold hide-mobile">ZOKASCORE</span></Link></div>
          ...
          <div className="nav-top-right">
            ...
            {isLoggedIn ? <Link to={ROUTES.PROFILE} className={`nav-profile-block ${isActive(ROUTES.PROFILE) ? 'is-active-profile' : ''}`}><div className="nav-avatar">{userProfile?.displayName?.[0]?.toUpperCase() || 'U'}</div><span className="font-bold hide-mobile text-sm">{userProfile?.displayName?.split(' ')[0] || 'User'}</span></Link> : <Link to={ROUTES.LOGIN} className="btn btn-primary btn-sm hide-mobile"><Zap size={16} /> Sign In</Link>}
