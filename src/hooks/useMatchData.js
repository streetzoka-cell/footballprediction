...
export { todayStr, yesterdayStr, tomorrowStr, getWeekStart, getMonthStart, eventBus, EVENT };

function safeEnsureUser(ensureFn, uid) {
  if (!uid || !ensureFn) return Promise.resolve();
  return Promise.race([
    ensureFn(uid),
...
export function useZokaPicks(date) {
  ...
  const { zokaPicks } = useAppData();
  const [picks, setPicks] = useState(null);

  useEffect(() => {
    if (isToday) setPicks(zokaPicks);
  }, [isToday, zokaPicks]);
  ...
  useEffect(() => {
    ...
    dataLayer.fetchZokaPicks(dateStr)
      ...
      .catch(() => { if (!cancelled) setPicks(null); });
    return () => { cancelled = true; };
  }, [dateStr, isToday]);

  return isToday ? zokaPicks : picks;
}

export function useZokaVotes(date) {
  const dateStr = date || todayStr();
  const isToday = dateStr === todayStr();
  const { zokaVoteStats, currentUserVotes } = useAppData();
  const [voteStats, setVoteStats] = useState({});
  const [userVotes, setUserVotes] = useState({});

  useEffect(() => {
    if (isToday) {
      setVoteStats(zokaVoteStats);
      setUserVotes(currentUserVotes);
    }
  }, [isToday, zokaVoteStats, currentUserVotes]);
  ...
  return {
    votes: [],
    voteStats: isToday ? zokaVoteStats : voteStats,
    userVotes: isToday ? currentUserVotes : userVotes,
  };
}
...
export function useActivePredictions(date) {
  const dateStr = date || todayStr();
  const isToday = dateStr === todayStr();
  const { activePredictions, loading } = useAppData();
  const [preds, setPreds] = useState(null);
  const [load, setLoad] = useState(!isToday);

  useEffect(() => {
    if (isToday) {
      setPreds(activePredictions);
      setLoad(loading);
    }
  }, [isToday, activePredictions, loading]);
  ...
  useEffect(() => {
    ...
    dataLayer.fetchActivePredictions(dateStr)
      .then((data) => { if (!cancelled) { setPreds(data); setLoad(false); } })
      .catch(() => { if (!cancelled) { setPreds([]); setLoad(false); } });
    return () => { cancelled = true; };
  }, [dateStr, isToday]);

  const scoreMap = useMemo(() => dataLayer.getScoreMap(isToday ? activePredictions : preds || []), [isToday, activePredictions, preds]);

  return {
    preds: isToday ? activePredictions : (preds || []),
    scoreMap,
    loading: isToday ? loading : load,
    error: null,
    lastUpdate: Date.now(),
  };
}
...
export function useMyPredictions(uid, date) {
  const dateStr = date || todayStr();
  const isToday = dateStr === todayStr();
  const { userPredictions, ensureUserData } = useAppData();
  const [preds, setPreds] = useState(null);

  useEffect(() => {
    if (uid) safeEnsureUser(ensureUserData, uid);
  }, [uid, ensureUserData]);

  useEffect(() => {
    if (isToday && uid) {
      setPreds(userPredictions);
    } else if (!uid) {
      setPreds({});
    }
  }, [isToday, uid, userPredictions]);

  useEffect(() => {
    if (isToday || !uid) return;
    let cancelled = false;
    dataLayer.fetchUserPredictions(uid, dateStr)
      .then((data) => { if (!cancelled) setPreds(data); })
      .catch(() => { if (!cancelled) setPreds({}); });
    return () => { cancelled = true; };
  }, [uid, dateStr, isToday]);

  return isToday ? (userPredictions || {}) : (preds || {});
}

export function usePredictionResults(uid) {
  const { predictionResults, ensureUserData } = useAppData();
  useEffect(() => {
    if (uid) safeEnsureUser(ensureUserData, uid);
  }, [uid, ensureUserData]);

  if (!uid) return { results: [], resultMap: {} };
  return predictionResults || { results: [], resultMap: {} };
}

export function useUserPoints(uid) {
  const { userPoints, ensureUserData } = useAppData();
  useEffect(() => {
    if (uid) safeEnsureUser(ensureUserData, uid);
  }, [uid, ensureUserData]);
  return uid ? userPoints : null;
}

export function useMyStats(uid) {
  const { userStats, ensureUserData } = useAppData();
  useEffect(() => {
    if (uid) safeEnsureUser(ensureUserData, uid);
  }, [uid, ensureUserData]);
...
export function useDailyLeaderboard(date) {
  const dateStr = date || todayStr();
  const isToday = dateStr === todayStr();
  const { dailyLeaderboard, loading: appLoading } = useAppData();
  const [local, setLocal] = useState({ entries: null, top3: null, rest: null, stats: null, loading: false });

  useEffect(() => {
    if (isToday) {
      const entries = dailyLeaderboard?.entries || [];
      setLocal({
        entries,
        top3: dailyLeaderboard?.top3 || entries.slice(0, 3),
        rest: dailyLeaderboard?.rest || entries.slice(3),
        stats: dailyLeaderboard?.stats || computeStats(entries),
        loading: appLoading,
      });
    }
  }, [isToday, dailyLeaderboard, appLoading]);
  ...
  useEffect(() => {
    ...
    dataLayer.fetchDailyLeaderboard(dateStr)
      ...
      .catch(() => {
        if (!cancelled) setLocal({ entries: [], top3: [], rest: [], stats: computeStats([]), loading: false });
      });
    return () => { cancelled = true; };
  }, [dateStr, isToday]);
  ...
  return {
    ...
    top3: local.top3 || [],
    rest: local.rest || [],
    stats: local.stats || computeStats([]),
    loading: isToday ? appLoading : local.loading,
    isLive: false,
  };
}

export function useHistoricalLeaderboard(period) {
  const { historicalLeaderboards, loadHistoricalLeaderboard } = useAppData();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const cachedData = historicalLeaderboards?.[period];
  ...
  useEffect(() => {
    ...
    loadHistoricalLeaderboard?.(period)
      .then(() => { if (!cancelled) { setLoading(false); setError(null); } })
      .catch(() => { if (!cancelled) { setLoading(false); setError('load_failed'); } });

    return () => { cancelled = true; clearTimeout(timer); };
  }, [period, cachedData, loadHistoricalLeaderboard]);

  const data = historicalLeaderboards?.[period] || cachedData;
...
export async function saveZokaVote(uid, matchId, vote) {
  ...
  await setDoc(doc(db, PATHS.ZOKA_VOTE_STATS, dateStr), {
...
async function _updateDailySummaryIncremental(dateStr, resolvedList) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    for (const r of resolvedList) {
      ...
      if (!entry) { entry = { uid: r.userId, displayName: 'Player', points: 0, predictions: 0, exact: 0, result: 0, miss: 0, resolved: 0 }; entryMap.set(r.userId, entry); }
      entry.displayName = r.displayName || entry.displayName || 'Player';
      entry.predictions += 1; entry.resolved += 1; entry.points += r.points;
      if (r.resultType === RESULT_TYPE.EXACT) entry.exact += 1; else if (r.resultType === RESULT_TYPE.RESULT) entry.result += 1; else entry.miss += 1;
    }
    const entries = rankEntries([...entryMap.values()].filter((u) => u.predictions > 0));
    transaction.set(ref, { entries, top3: entries.slice(0, 3), rest: entries.slice(3), stats: computeStats(entries), scoreMap, predDist: base?.predDist || {}, predCounts: base?.predCounts || {}, date: dateStr, updatedAt: serverTimestamp() }, { merge: true });
  });
...
async function _updateGoatIncremental(resolvedList) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    for (const r of resolvedList) {
      ...
      if (!entry) { entry = { uid: r.userId, displayName: 'Player', points: 0, predictions: 0, exact: 0, result: 0, miss: 0, resolved: 0 }; entryMap.set(r.userId, entry); }
      entry.displayName = r.displayName || entry.displayName || 'Player';
      entry.points += r.points; entry.predictions += 1; entry.resolved += 1;
      if (r.resultType === RESULT_TYPE.EXACT) entry.exact += 1; else if (r.resultType === RESULT_TYPE.RESULT) entry.result += 1; else entry.miss += 1;
    }
    const entries = rankEntries([...entryMap.values()].filter((u) => u.predictions > 0));
    transaction.set(ref, { entries, top3: entries.slice(0, 3), rest: entries.slice(3), stats: computeStats(entries), updatedAt: serverTimestamp() }, { merge: true });
  });
  ...
  try {
    ...
    predsSnap.forEach((d) => {
      try {
        ...
        ops += 2;
        if (ops >= 450) { // Firestore batch limit safeguard
          batch.commit();
          batch = writeBatch(db);
          ops = 0;
        }
