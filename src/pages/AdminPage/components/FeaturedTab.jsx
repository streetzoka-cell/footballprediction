    ...
    <div className="flex-col gap-16">
        ...
        {preds.length > 0 ? (
          <div className="flex-col gap-8">
            {preds.map((p, i) => {
              ...
              return (
                <div key={mid} className="glass-card p-12 flex-col gap-8 anim-fade-up group hover:border-primary/20 transition-colors" style={{ animationDelay: `${i * 20}ms`, borderLeft: '3px solid var(--primary)' }}>
                  ...
                  <div className="flex-center gap-8">
                    <div className="flex-center gap-8 flex-1 min-w-0 justify-end">
                      <span className="text-primary font-bold text-sm truncate text-right">{p.homeTeam?.shortName || p.homeTeam?.name || 'Home'}</span>
      ...
      {!isFull ? (
        ...
        <div className="glass-card p-24 flex-col items-center gap-8 text-center anim-fade-up" style={{ borderColor: 'rgba(var(--gold-rgb), 0.2)' }}>
          ...
          <p className="text-muted text-sm max-w-400">You have reached the maximum of {MAX_FEATURED} featured matches. Remove some matches above to add new ones.</p>
        </div>
      )}
    </div>
  );
