      ...
      {unresolved.length > 0 && (
        <div className="glass-card p-16 flex flex-col gap-12">
          ...
          {unresolved.map((p, i) => {
            ...
            return (
              <div key={mid} className="glass-card p-12 flex flex-col gap-8 anim-fade-up" style={{ animationDelay: `${i * 20}ms`, borderLeft: `3px solid ${hasExisting ? 'var(--warning)' : 'var(--accent)'}` }}>
                <div className="flex-between">
      ...
      {resolved.length > 0 && (
        <div className="glass-card p-16 flex flex-col gap-12">
          ...
          {resolved.map((p, i) => {
            ...
            return (
              <div key={mid} className="glass-card p-12 flex flex-col gap-8 anim-fade-up opacity-80" style={{ animationDelay: `${i * 15}ms`, borderLeft: '3px solid var(--primary)' }}>
                <div className="flex-between">
                  <div className="flex-center gap-8 text-muted text-xs font-bold">
                    {p.league?.emblem && <img src={p.league.emblem} alt="" width="14" height="14" />}
                    <span>{p.league?.name || 'Match'}</span>
