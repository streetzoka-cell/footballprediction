  ...
  return (
    <div className="flex flex-col gap-16">
      <div className="glass-card p-16 flex flex-col gap-12">
        ...
        <div className="admin-grid-120">
          <div className="glass-card p-12 flex flex-col items-center gap-4"><span className="font-extrabold text-primary">{apiSuccessRate}%</span><span className="text-muted text-xs">API Success</span></div>
          <div className="glass-card p-12 flex flex-col items-center gap-4"><span className="font-extrabold text-accent">{cacheHitRatio}%</span><span className="text-muted text-xs">Cache Hit Ratio</span></div>
          <div className="glass-card p-12 flex flex-col items-center gap-4"><span className="font-extrabold text-danger">{errorLogs.length}</span><span className="text-muted text-xs">Active Errors</span></div>
          <div className="glass-card p-12 flex flex-col items-center gap-4"><span className="font-extrabold text-gold">{Object.keys(apiMetrics).length}</span><span className="text-muted text-xs">Tracked Endpoints</span></div>
        </div>
      </div>
      ...
      <div className="glass-card p-16 flex flex-col gap-12">
        ...
        <div className="flex flex-col gap-8 max-h-400px">
          {errorLogs.length === 0 ? (
            <Empty icon={Activity} title="No errors detected" hint="The application is running smoothly." />
          ) : (
            errorLogs.map((log, i) => (
              <div key={i} className="glass-card p-12 flex flex-col gap-4">
                <div className="flex-between">
