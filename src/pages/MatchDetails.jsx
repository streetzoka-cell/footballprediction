...
export default function MatchDetails() {
  ...
  const intelData = useMemo(() => match?.intelData || null, [match]);
  ...
  return (
    <div className="md-page">
      <SEO {...seo} />
      {match && <h1 className="sr-only">{match.homeName} vs {match.awayName} - {match.leagueName} Live Score & Statistics</h1>}

      {match?.isHidden ? (
        <div className="zoka-page flex-center">
          <div className="glass-card empty-card"><Clock size={32} className="gold" /><h2>Match Temporarily Unavailable</h2><p className="text-muted">Waiting for final confirmation.</p><Link to="/fixtures" className="btn btn-ghost"><ArrowLeft size={14} /> Back to Fixtures</Link></div>
        </div>
      ) : !match ? (
        <div className="zoka-page flex-center"><div className="flex-col gap-16 w-full max-w-800"><div className="skeleton" style={{ height: 200, borderRadius: 24 }} /><div className="skeleton" style={{ height: 100, borderRadius: 16 }} /></div></div>
      ) : (
        <div className="md-container">
          <Link to="/fixtures" className="btn btn-ghost btn-sm mb-12"><ArrowLeft size={14} /> Back</Link>
          ...
          <div className={`md-header-card ${goalFlash ? 'goal-flash' : ''}`}>
            ...
            <div className="flex-center gap-8 mb-16">
              ...
              <Link to={buildLeagueRoute(match.leagueId, match.leagueName)} className="text-muted font-bold text-xs hover-primary">{match.leagueName}</Link>
              {match.category === 'FEATURED' && <span className="badge badge-gold">★ TOP</span>}
            </div>
            ...
            {(match.isLive || match.isFinished) && (
              <div className="md-timeline"><div className="md-timeline-fill" style={{ width: `${match.timelineProgress}%` }} /></div>
            )}
          </div>

          {match.aiPreview && (
            <div className="glass-card ai-preview-card"><h2 className="ai-preview-title"><Brain size={16} /> Zoka AI Tactical Preview</h2><p className="text-muted text-sm">{match.aiPreview}</p></div>
          )}
          ...
          <div className="md-pro-grid">
            <div className="glass-card p-12">
              ...
              <div className="md-mini-list">
                {match.date && <div className="md-mini-row"><span className="text-muted flex-center gap-4"><Calendar size={12} /> Kickoff</span><span className="font-bold">{new Date(match.date).toLocaleDateString('en-GB', { weekday: 'short', day: 'numeric', month: 'short' })} · {formatTime(match.date)}</span></div>}
                {match.venue?.name && <div className="md-mini-row"><span className="text-muted flex-center gap-4"><MapPin size={12} /> Venue</span><span className="font-bold">{match.venue.name}</span></div>}
                {match.referee && <div className="md-mini-row"><span className="text-muted flex-center gap-4"><Shield size={12} /> Referee</span><span className="font-bold">{match.referee}</span></div>}
              </div>
            </div>
            <div className="glass-card p-12">
              <h3 className="md-mini-title"><Users size={12} /> Top 3 Standings</h3>
              {standingsTable.length > 0 ? standingsTable.slice(0,3).map((team,i) => (
                <Link key={team.team?.id || i} to={buildTeamRoute(team.team?.id, team.team?.name)} className="md-mini-row hover-primary">
                  <span className="rank">{team.rank || i+1}</span>
                  <span className="flex-center gap-4 flex-1 truncate">{team.team?.logo && <img src={team.team?.logo} alt="" width="12" height="12" />}{team.team?.name}</span>
          ...
          {match.hasRealStats ? (
            <div className="glass-card p-16 mt-12"><h2 className="md-section-title"><BarChart3 size={16} /> Statistics</h2>
            ...
            <div className="glass-card pending-card"><Activity size={20} className="primary" /><h3>Tactical Insight Pending</h3><p className="text-muted text-xs">{match.isLive ? 'Live stats tracking — ask AI for insights!' : 'Stats appear once match begins.'}</p><button onClick={() => window.dispatchEvent(new CustomEvent('openZokaAI', { detail: { message: `Give me tactical breakdown for ${match.homeName} vs ${match.awayName} in ${match.leagueName}.` } }))} className="btn btn-primary btn-sm"><Zap size={14} /> Ask Zoka AI</button></div>
          )}
          ...
          <div className="glass-card p-16 mt-12">
            ...
            {match.odds && (match.odds.home || match.odds.away) ? (
              <div className="odds-grid"><div className="odd-box home"><div className="lbl">Home</div><div className="val">{match.odds.home || '-'}</div></div><div className="odd-box draw"><div className="lbl">Draw</div><div className="val gold">{match.odds.draw || '-'}</div></div><div className="odd-box away"><div className="lbl">Away</div><div className="val danger">{match.odds.away || '-'}</div></div></div>
            ) : <div className="text-muted text-xs text-center py-8 flex-center gap-8"><Shield size={16} /> Odds unavailable.</div>}
          </div>

          <div className="glass-card p-16 mt-12">
            <h2 className="md-section-title"><TrendingUp size={16} /> H2H & Recent Form</h2>
            <div className="h2h-grid">
              <div><h3 className="h2h-team-title"><img src={match.homeLogo} alt="" width="12" height="12" />{match.homeName}</h3><ul className="h2h-list">{homeResults.length ? homeResults.map(m => <li key={m.id}><Link to={buildMatchRoute(m.id, m.homeName || m.homeTeam?.name, m.awayName || m.awayTeam?.name)} className="h2h-link"><ChevronRight size={12} />{m.homeName || m.homeTeam?.name} vs {m.awayName || m.awayTeam?.name}<span className="font-extrabold">{m.homeScore} - {m.awayScore}</span></Link></li>) : <li className="text-muted text-xs">No recent results.</li>}</ul></div>
              <div><h3 className="h2h-team-title"><img src={match.awayLogo} alt="" width="12" height="12" />{match.awayName}</h3><ul className="h2h-list">{awayResults.length ? awayResults.map(m => <li key={m.id}><Link to={buildMatchRoute(m.id, m.homeName || m.homeTeam?.name, m.awayName || m.awayTeam?.name)} className="h2h-link"><ChevronRight size={12} />{m.homeName || m.homeTeam?.name} vs {m.awayName || m.awayTeam?.name}<span className="font-extrabold">{m.homeScore} - {m.awayScore}</span></Link></li>) : <li className="text-muted text-xs">No recent results.</li>}</ul></div>
            </div>
          </div>
        </div>
      )}
