...
//   CheckCircle2, XCircle, AlertTriangle, Loader2, Plus, Trash2, Save, Send, Pencil, Copy, Check, TrendingUp, History, Cpu, Database, CalendarDays, Crown, Timer, Sparkles, Zap, Ban, Eye, MousePointerClick
// } from 'lucide-react';
...
// import { todayStr, getLocalDateStr, getLocalDateFromUtc, parseDateAsUTC } from '../utils/dates';
// import { eventBus, EVENT } from '../utils/eventBus';
// import { PATHS, isLiveStatus, isFinishedStatus, SPORT } from '../utils/constants';
...
// import { collection, query, where, onSnapshot, doc, setDoc, deleteDoc, serverTimestamp, getDocs, limit as limitQ, startAfter, addDoc } from 'firebase/firestore';
...
// const useMounted = () => { const r = useRef(true); useEffect(() => () => { r.current = false; }, []); return r; };
...
// const sortByImportance = (a, b) => (b.matchScore || 0) - (a.matchScore || 0);
...
// const ST_MAP = {
//   SCHEDULED:{c:'var(--text-muted)',b:'rgba(255,255,255,.04)',l:'Upcoming'}, TIMED:{c:'var(--text-muted)',b:'rgba(255,255,255,.04)',l:'Upcoming'},
//   NS:{c:'var(--text-muted)',b:'rgba(255,255,255,.04)',l:'Upcoming'}, TBD:{c:'var(--text-muted)',b:'rgba(255,255,255,.04)',l:'TBD'},
//   IN_PLAY:{c:'#ef4444',b:'rgba(239,68,68,.1)',l:'Live'}, PAUSED:{c:'#f97316',b:'rgba(249,115,22,.1)',l:'HT'},
//   '1H':{c:'#ef4444',b:'rgba(239,68,68,.1)',l:'Live'}, '2H':{c:'#ef4444',b:'rgba(239,68,68,.1)',l:'Live'},
//   HT:{c:'#f97316',b:'rgba(249,115,22,.1)',l:'HT'}, BT:{c:'#f97316',b:'rgba(249,115,22,.1)',l:'BT'},
//   ET:{c:'#ef4444',b:'rgba(239,68,68,.1)',l:'ET'}, P:{c:'#ef4444',b:'rgba(239,68,68,.1)',l:'Pens'},
//   FT:{c:'var(--accent)',b:'rgba(16,185,129,.08)',l:'FT'}, FINISHED:{c:'var(--accent)',b:'rgba(16,185,129,.08)',l:'FT'},
//   AET:{c:'var(--accent)',b:'rgba(16,185,129,.08)',l:'FT'}, PEN:{c:'var(--accent)',b:'rgba(16,185,129,.08)',l:'FT'},
//   PST:{c:'#f59e0b',b:'rgba(245,158,11,.1)',l:'PST'},
// };
// const gst = s => ST_MAP[s] || ST_MAP.SCHEDULED;
// const isLive = m => isLiveStatus(m?.status, m?.sport || SPORT.FOOTBALL) || m?.isLive;
// const isFin = m => isFinishedStatus(m?.status, m?.sport || SPORT.FOOTBALL) || m?.isFinished;
...
//   const [stats, setStats] = useState({ totalUsers: 0, totalPredictions: 0 });
...
//         setStats({ totalUsers: userSnap.size, totalPredictions: 0 });
//       } catch (e) { toast('Analytics load failed: ' + e.message, 'er'); }
//       setLoading(false);
//     };
//     fetchStats();
//   }, [toast]);
...
//             <span className="l">{stats.totalUsers}</span>
//           </div>
//           <div className="astat">
//             <span className="n gn"><MousePointerClick size={18} /></span>
//             <span className="l">Interactions</span>
//           </div>
//           <div className="astat">
...
//       <div className="ae">
//         <div className="asec">
//           <Empty icon={AlertTriangle} title="Backend Logs Unavailable" hint="Ensure your backend server is running and the /api/v1/system/status route is active." />
//         </div>
//       </div>
//     );
//   }
...
//             <span style={{ color: job.status === 'success' ? '#10b981' : job.status === 'error' ? '#ef4444' : '#64748b' }}>● {name}</span>
//             <span className="abdg pn" style={{ marginLeft: 'auto' }}>{job.status === 'running' ? 'Syncing...' : job.lastSync ? `Last: ${new Date(job.lastSync).toLocaleTimeString()}` : 'Never'}</span>
//           </div>
//         ))}
//       </div>
//     </div>
...
//       if (Object.keys(prev).length < MAX_ZOKA) {
...
//       }
//       toast(`Max ${MAX_ZOKA} Zoka Picks`, 'in'); return prev;
//     });
//   }, [pubMap, toast]);
...
//             <div>
//               <span style={{ fontSize: '.8rem', fontWeight: 700, color: 'var(--text-muted)' }}>{cnt}/{MAX_ZOKA} selected{scored === cnt && cnt > 0 && <span style={{ color: 'var(--accent)', marginLeft: 6 }}>✓ All scored</span>}</span>
//               {Object.keys(sel).some(mid => pubMap.has(mid)) && (<div className="aedit-hint"><Pencil size={9} /> Editing {Object.keys(sel).filter(mid => pubMap.has(mid)).length} existing</div>)}
//             </div>
//             <div style={{ display: 'flex', gap: 5 }}>
//               <button className="ab ab-gh ab-sm" onClick={handleSave} disabled={saving || cnt === 0 || scored === 0}><Save size={12} /> Save</button>
//               <button className="ab ab-gd ab-sm" onClick={handlePublish} disabled={publishing || !ready} title={!ready ? 'Enter scores for all picks to publish' : ''}><Send size={12} /> Publish</button>
//             </div>
//           </div>
//         </div>
//       )}
...
//           {leagues.map(l => (<button key={l.id} className={`alp${lg === l.id ? ' on' : ''}`} onClick={() => setLg(l.id)}>{l.emblem && <img src={l.emblem} alt="" onError={e => { e.target.style.display = 'none'; }} />}{l.name} ({l.n})</button>))}
//         </div>
//       )}
...
//             return (
//               <div key={mid}>
//                 <MatchRow m={m} idx={i} mode="zoka" sel={sel[mid]} onToggleSel={toggle} scoreInput={sel[mid]} onScoreInput={updScore} pubPick={isPublished ? pubMap.get(mid) : null} extraBadge={isPublished && !sel[mid] ? (<span className="abdg gd"><Star size={9} /> Published</span>) : null} />
//                 {isPublished && !sel[mid] && (<div className="aedit-hint" style={{ margin: '-4px 16px 8px', cursor: 'pointer' }} onClick={() => toggle(m)}><Pencil size={9} /> Tap to edit published pick: {pubMap.get(mid).adminPick?.home}-{pubMap.get(mid).adminPick?.away}</div>)}
//               </div>
//             );
//           })}
...
//         <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => { setShowHist(p => !p); if (!showHist) loadHist(); }}>
...
//                   <div className="amh">
//                     <div className="aml">{p.league?.emblem && <img src={p.league.emblem} alt="" onError={e => { e.target.style.display = 'none'; }} />}<span>{p.league?.name || 'Featured'}</span></div>
//                     <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>{live && <span className="ld" />}<span className="as" style={{ color: st.c, background: st.b }}>{st.l}</span></div>
//                   </div>
//                   <div className="atm">
//                     <div className="ate">{(p.homeLogo || p.homeTeam?.logo || p.homeTeam?.crest) && <img src={p.homeLogo || p.homeTeam?.logo || p.homeTeam?.crest} alt="" onError={e => { e.target.style.display = 'none'; }} />}<span>{p.homeTeam?.shortName || p.homeTeam?.name || 'Home'}</span></div>
//                     <div className={`asb${live ? ' lv' : ''}${finished ? ' ft' : ''}`}>{sc ? (<><span className={`asn${live ? ' r' : ' g'}`}>{sc.h}</span><span className="asep">–</span><span className={`asn${live ? ' r' : ' g'}`}>{sc.a}</span></>) : <span className="avs">VS</span>}</div>
//                     <div className="ate aw">{(p.awayLogo || p.awayTeam?.logo || p.awayTeam?.crest) && <img src={p.awayLogo || p.awayTeam?.logo || p.awayTeam?.crest} alt="" onError={e => { e.target.style.display = 'none'; }} />}<span>{p.awayTeam?.shortName || p.awayTeam?.name || 'Away'}</span></div>
//                   </div>
//                   <div className="aa">
//                     <span className="abdg gn"><Radio size={9} /> Featured</span>
//                     <button className="ab ab-sm ab-dg" onClick={() => handleRemoveClick(p)} disabled={isRemoving}>{isRemoving ? <Loader2 size={11} className="asp" /> : <Trash2 size={11} />} Remove</button>
//                   </div>
//                 </div>
//               );
//             })}
...
//               return (<MatchRow key={mid} m={m} idx={i} mode="featured" isFeatured={isFeatured} isAdding={isAdding} isFull={isFull} onAddClick={handleAddClick} onRemoveClick={handleRemoveClick} isRemoving={removingId === mid} />);
//             })}
//             <ShowMore count={hidden} show={showAll} onToggle={() => setShowAll(p => !p)} />
//           </div>
...
//             <button className="ab ab-sm ab-p" onClick={handleResolveAll} disabled={Object.values(resolving).some(Boolean)}><Zap size={11} /> Resolve All Scored</button>
//           </div>
//           {unresolved.map((p, i) => {
//             const mid = String(p.matchId || p.id);
//             const s = scores[mid] || {};
...
//                 <div className="amh">
//                   <div className="aml">{p.league?.emblem && <img src={p.league.emblem} alt="" onError={e => { e.target.style.display = 'none'; }} />}<span>{p.league?.name || 'Match'}</span></div>
...
//                 </div>
//                 <div className="aa">
//                   {hasExisting ? (<button className="ab ab-sm ab-olive" onClick={() => handleOverride(p)} disabled={overriding[mid] || (!s.h && s.h !== '0') || (!s.a && s.a !== '0')}>{overriding[mid] ? <Loader2 size={11} className="asp" /> : <Copy size={11} />} Override</button>) : (<button className="ab ab-sm ab-p" onClick={() => handleResolve(p)} disabled={isResolving || (!s.h && s.h !== '0') || (!s.a && s.a !== '0')}>{isResolving ? <Loader2 size={11} className="asp" /> : <Check size={11} />} Resolve</button>)}
...
//                   <button className="ab ab-sm ab-olive" onClick={() => handleOverride(p)} disabled={isOverriding || (!s.h && s.h !== '0') || (!s.a && s.a !== '0')}>{isOverriding ? <Loader2 size={11} className="asp" /> : <Copy size={11} />} Override</button>
//                 </div>
//               </div>
//             );
//           })}
...
//     setType('personal'); setUid(u.id); setSearch(`${u.displayName || u.email || u.id}`); setShowUserList(false);
...
//           <button className={`ab ab-sm ${type === 'global' ? 'ab-p' : 'ab-ol'}`} onClick={() => setType('global')} style={{ flex: 1 }}><Users size={12} /> Global</button>
//           <button className={`ab ab-sm ${type === 'personal' ? 'ab-p' : 'ab-ol'}`} onClick={() => setType('personal')} style={{ flex: 1 }}><UserCog size={12} /> Personal</button>
//         </div>
//         {type === 'personal' && (
...
//               <button className="ab ab-bl ab-sm" onClick={loadUsers} disabled={loadingUsers} style={{ flexShrink: 0 }}>{loadingUsers ? <Loader2 size={11} className="asp" /> : <Users size={11} />} Load Users</button>
//             </div>
//             {showUserList && (
...
//                     <div style={{ width: 30, height: 30, borderRadius: 8, background: 'var(--bg-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.65rem', fontWeight: 800, color: '#fff' }}>{(u.displayName || u.email || '??').slice(0, 2).toUpperCase()}</div>
//                     <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: '.76rem', fontWeight: 700, color: 'var(--text-primary)' }}>{u.displayName || 'Anonymous'}</div><div style={{ fontSize: '.62rem', color: 'var(--text-muted)' }}>{u.email || u.id}</div></div>
//                   </div>
...
//           </div>
//         )}
//         <div style={{ marginBottom: 12 }}><input className="aip" placeholder="Notification Title..." value={title} onChange={e => setTitle(e.target.value)} /></div>
//         <div style={{ marginBottom: 14 }}><textarea className="aip" placeholder="Message body..." value={message} onChange={e => setMessage(e.target.value)} rows={4} style={{ resize: 'vertical', minHeight: 100 }} /></div>
//         <button className="ab ab-p" onClick={handleSend} disabled={sending || !title.trim() || !message.trim()} style={{ width: '100%' }}>{sending ? <Loader2 size={13} className="asp" /> : <Send size={13} />} Broadcast Message</button>
//       </div>
//     </div>
//   );
// });
...
//           <button className="ab ab-p ab-sm" onClick={addStaff} disabled={adding || !email.trim()}>{adding ? <Loader2 size={11} className="asp" /> : <Plus size={11} />} Add</button>
//         </div>
//         <button className="ab ab-gh ab-sm" onClick={loadStaff} disabled={loading} style={{ marginBottom: 12 }}>{loading ? <Loader2 size={11} className="asp" /> : <Users size={11} />} {staff.length > 0 ? 'Refresh' : 'Load Staff from Firebase'}</button>
//         {staff.length > 0 ? staff.map(s => (
//           <div key={s.id} className="aur">
//             <div style={{ width: 38, height: 38, borderRadius: 10, background: s.role === 'admin' ? 'rgba(245,197,66,.12)' : 'rgba(96,165,250,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: s.role === 'admin' ? 'var(--gold)' : 'var(--blue)', fontWeight: 900, fontSize: '.85rem', flexShrink: 0 }}>{(s.displayName || s.email || '??').slice(0, 2).toUpperCase()}</div>
//             <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: '.84rem', fontWeight: 700, color: 'var(--text-primary)' }}>{s.displayName || 'Unknown'}</div><div style={{ fontSize: '.68rem', color: 'var(--text-muted)', fontWeight: 600 }}>{s.email}</div></div>
...
//             <div style={{ width: 38, height: 38, borderRadius: 10, background: `hsl(${(i * 37) % 360}, 50%, 25%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, fontSize: '.78rem', flexShrink: 0 }}>{(u.displayName || u.email || '??').slice(0, 2).toUpperCase()}</div>
//             <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: '.82rem', fontWeight: 700, color: 'var(--text-primary)' }}>{u.displayName || 'Anonymous'}</div><div style={{ fontSize: '.66rem', color: 'var(--text-muted)', fontWeight: 600 }}>{u.email || u.id}</div></div>
//             <div style={{ textAlign: 'right', flexShrink: 0 }}><div style={{ fontSize: '.68rem', fontWeight: 700, color: u.role === 'admin' ? 'var(--gold)' : u.role === 'staff' ? 'var(--blue)' : 'var(--text-muted)' }}>{(u.role || 'user').toUpperCase()}</div><div style={{ fontSize: '.6rem', color: 'var(--text-muted)', fontWeight: 600 }}>{u.createdAt ? fmtTimeAgo(u.createdAt) : ''}</div></div>
//           </div>
//         )) : !loading && <Empty icon={Users} title="No users found" />}
//         {hasMore && (<button className="asm" onClick={() => loadUsers(true)} disabled={loading} style={{ marginTop: 8 }}>{loading ? <Loader2 size={13} className="asp" /> : <ChevronDown size={13} />} Load more</button>)}
//       </div>
//     </div>
//   );
// });
...
//       <div className="ae">
//         <div className="asec">
//           <Empty icon={AlertTriangle} title="System Health Unavailable" hint="Cannot connect to the /health endpoint. Make sure the backend is online." />
//         </div>
//       </div>
//     );
//   }
...
//         <div className="asg">
//           <div className="astat"><span className="n" style={{ color: '#10b981' }}>{health?.status === 'healthy' ? '🟢' : '🔴'}</span><span className="l">Overall Status</span></div>
//           <div className="astat"><span className="n bl">{health?.budget?.football ?? 'N/A'}</span><span className="l">Football Budget</span></div>
//           <div className="astat"><span className="n gn">{health?.budget?.basketball ?? 'N/A'}</span><span className="l">Basketball Budget</span></div>
//           <div className="astat"><span className="n gd">{health?.uptime ? Math.round(health.uptime / 60) : 0} min</span><span className="l">Uptime</span></div>
//         </div>
//       </div>
//       <div className="asec">
//         <h3 className="ast"><Database size={15} /> Cache Stats</h3>
//         <div className="aur"><span style={{ color: 'var(--text-muted)' }}>Total Keys</span><span className="abdg bl">{health?.cache?.keys || 0}</span></div>
//         <div className="aur"><span style={{ color: 'var(--text-muted)' }}>Hits</span><span className="abdg gn">{health?.cache?.hits || 0}</span></div>
//         <div className="aur"><span style={{ color: 'var(--text-muted)' }}>Misses</span><span className="abdg rd">{health?.cache?.misses || 0}</span></div>
//       </div>
//     </div>
//   );
// });
...
//     const dates = [];
//     for (let i = -14; i <= 14; i++) { const d = getLocalDateStr(i); if (!defaultDates.includes(d)) dates.push(d); }
//     return dates.sort();
//   }, [defaultDates]);
...
//   useEffect(() => {
//     if (!db) return;
//     const unsub = onSnapshot(doc(db, PATHS.PREDICTION_SNAPSHOTS, date), (snap) => {
...
//           .then(qs => { if (mounted.current) setPreds(qs.docs.map(d => d.data()).sort((a, b) => (b.priority || 0) - (a.priority || 0))); })
//           .catch(() => {});
//       }
//     }, () => {});
//     return unsub;
...
//   useEffect(() => {
//     if (!db) return;
//     const unsub = onSnapshot(doc(db, PATHS.ZOKA_PICKS, date), (snap) => {
//       if (!mounted.current) return;
...
//     const pred = { id: predId, matchId: String(m.id), matchDate: date, homeTeam: m.homeTeam, awayTeam: m.awayTeam, homeLogo: m.homeTeam?.crest || null, awayLogo: m.awayTeam?.crest || null, league: m.competition || m.league, kickoff: m.utcDate || m.kickoff, status: m.status || 'NS', homeScore: null, awayScore: null, priority: preds.length + 1 };
...
//     } catch (e) { showToast('Rebuild failed', 'er'); }
//     setRebuilding(null);
//   }, [date, showToast]);
...
//       <div className="aw">
//         <div className="ah">
//           <button className="ab ab-gh ab-sm" onClick={() => nav('/')} style={{ position: 'absolute', left: 16, top: 20 }}><ArrowLeft size={14} /></button>
...
//           <div className="adb">
//             {defaultDates.map(d => (<button key={d} className={`adp${d === date ? ' on' : ''}`} onClick={() => setDate(d)}>{dateLabel(d)}</button>))}
//             <button className="more-dates-btn" onClick={() => setShowMoreDates(p => !p)}>{showMoreDates ? <ChevronUp size={12} /> : <ChevronDown size={12} />}{showMoreDates ? 'Less' : 'More dates'}</button>
//             {showMoreDates && extraDates.map(d => (<button key={d} className={`adp past${d === date ? ' on' : ''}`} onClick={() => setDate(d)}>{dateLabel(d)}</button>))}
//           </div>
//         </div>
...
//         {tab === 'featured' && (<FeaturedTab date={date} preds={preds} fixtures={allFixtures} onAdd={handleFeaturedAdd} onRemove={handleFeaturedRemove} fxLoading={fxLoading} toast={showToast} />)}
