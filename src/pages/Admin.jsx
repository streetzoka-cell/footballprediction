...
export default function Admin() {
  ...
  // Local overrides — set after writes so UI updates instantly
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToTodayFixtures(({ matches, error }) => {
      ...
      // Update active_predictions with finished scores
  ...
  useEffect(() => {
    resolvedMatchesRef.current = new Set();
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        list.sort((a, b) => {
          const o = { admin: 0, lead: 1, analyst: 2, writer: 3 };
          return (o[a.role] || 9) - (o[b.role] || 9);
        });
        setStaffList(list);
        setStaffLoad(false);
      },
      (err) => {
        console.error('[Admin] Staff snapshot:', err);
        setStaffLoad(false);
      }
    );
    return () => unsub();
  ...
  const handleAdd = async (match) => {
    ...
    try {
      await setDoc(doc(db, 'active_predictions', `${date}_${match.id}`), {
        ...
        kickoff: match.kickoff,
        status: 'upcoming',
        priority: MAX_FEATURED - preds.length,
        homeScore: null,
        awayScore: null,
