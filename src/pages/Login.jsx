...
// ─── OPTIMIZATION: Static styles moved outside component ───
const STYLES = {
  pageWrap: {
    minHeight: '100vh',
    background: '#05070a',
  ...
  googleBtn: {
    ...
    background: 'rgba(255,255,255,.04)', border: '1.5px solid #151b26',
    color: '#f8fafc', fontWeight: 700, fontSize: '.95rem',
    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12,
    marginBottom: 24, minHeight: 54,
    transition: 'all .18s cubic-bezier(.22,1,.36,1), opacity .2s',
  },
  dividerLine: { flex: 1, height: 1.5, background: '#151b26', borderRadius: 1 },
  submitBtn: {
    width: '100%', height: 58, fontSize: '1.05rem', position: 'relative',
    overflow: 'hidden', borderRadius: 16,
    background: 'linear-gradient(135deg, #10b981 0%, #059669 50%, #047857 100%)',
    color: '#05070a', fontWeight: 900, border: 'none',
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <input
          ...
          style={{
            width: '100%', padding: '16px 50px 16px 50px', borderRadius: 14,
            background: focused ? 'rgba(16,185,129,.03)' : '#0d1118',
            border: `2px solid ${focused ? '#10b981' : '#151b26'}`,
            color: '#f8fafc', fontSize: '1rem', fontWeight: 600, outline: 'none',
            transition: 'border-color .2s, box-shadow .2s, background .2s',
            boxShadow: focused ? '0 0 0 4px rgba(16,185,129,.1)' : 'none',
            boxSizing: 'border-box', minHeight: 56, WebkitAppearance: 'none', appearance: 'none',
          }}
        />
        {isPassword && (
          <button
            ...
            style={{
              ...
              color: focused ? '#10b981' : '#64748b', padding: 6, background: 'none', border: 'none',
              cursor: 'pointer', transition: 'color .2s', display: 'flex', alignItems: 'center', justifyContent: 'center',
              width: 40, height: 40, borderRadius: 10, WebkitTapHighlightColor: 'transparent',
            }}
...
const PitchDecoration = memo(function PitchDecoration() {
  return (
    <svg width="100%" height="100%" style={{ position: 'absolute', inset: 0, opacity: 0.018, pointerEvents: 'none' }} preserveAspectRatio="none" viewBox="0 0 400 600">
...
export default function Login() {
  ...
  // ─── OPTIMIZATION: Lightweight Loader while Firebase checks auth state ───
  if (authLoading) {
    return (
      <div style={{ minHeight: '100vh', background: '#05070a', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Loader size={32} style={{ animation: 'auth_spin .7s linear infinite', color: '#10b981' }} />
      </div>
    );
  }
...
/>
      ...
      <div className="auth-pop auth-glow" style={STYLES.card}>
        <div style={STYLES.shineOverlay} />

        <div style={STYLES.headerWrap}>
          <div className="auth-float" style={STYLES.logoBox}>
          ...
          <h2 style={{
            margin: 0, fontSize: '1.7rem', fontWeight: 900, color: '#f8fafc', letterSpacing: '.01em',
            opacity: modeTrans ? 0 : 1, transform: modeTrans ? 'translateY(-8px)' : 'translateY(0)',
            transition: `opacity .2s, transform .2s`,
          }}>
            {isLogin ? 'Welcome Back' : 'Create Account'}
          </h2>
          <p style={{
            margin: '8px 0 0', fontSize: '.95rem', fontWeight: 600, color: '#64748b',
            opacity: modeTrans ? 0 : 1, transform: modeTrans ? 'translateY(-8px)' : 'translateY(0)',
        ...
        {error && (
          <div style={{
            padding: '16px 18px', background: 'rgba(239,68,68,.08)', border: '1.5px solid rgba(239,68,68,.2)',
            borderRadius: 14, color: '#ef4444', fontSize: '.92rem', fontWeight: 700, marginBottom: 24,
            display: 'flex', alignItems: 'center', gap: 12,
            opacity: errorVis ? 1 : 0, transform: errorVis ? 'translateY(0)' : 'translateY(-10px)',
            transition: 'opacity .3s, transform .3s', animation: errorVis ? 'auth_slideErr .3s ease both' : 'none',
          }}>
            <span style={{ flex: 1, lineHeight: 1.4 }}>{error}</span>
            <button onClick={() => setError('')} className="zoka-btn" style={{ background: 'rgba(239,68,68,.1)', border: 'none', color: '#ef4444', padding: 6, display: 'flex', borderRadius: 8, width: 32, height: 32, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} aria-label="Dismiss error">
              <X size={16} />
            </button>
          </div>
        )}

        <button onClick={handleGoogle} disabled={loading} className="zoka-btn" style={{ ...STYLES.googleBtn, opacity: loading ? .5 : 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
          ...
          <span style={{ fontSize: '.82rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '.1em' }}>or use email</span>
          <div style={STYLES.dividerLine} />
        </div>

        <form onSubmit={handleSubmit}>
          <div style={{ overflow: 'hidden', maxHeight: isLogin ? '0px' : '140px', opacity: isLogin ? 0 : 1, transition: `max-height .45s ${EASE_OUT}, opacity .35s` }}>
          ...
          {isLogin && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 24, opacity: modeTrans ? 0 : 1, transition: 'opacity .2s' }}>
              <button type="button" className="zoka-btn" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#10b981', fontSize: '.9rem', fontWeight: 700, padding: '4px 0' }}>Forgot password?</button>
            </div>
          )}

          <button type="submit" className="zoka-btn" disabled={loading} style={{ ...STYLES.submitBtn, marginTop: isLogin ? 0 : 8 }}>
            <div style={{ position: 'absolute', top: 0, left: '-100%', width: '50%', height: '100%', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,.15), transparent)', animation: 'auth_shine 4s ease-in-out 1s infinite', pointerEvents: 'none' }} />
            <span style={{
              opacity: loading ? 0 : 1, transform: loading ? 'translateY(8px)' : 'translateY(0)',
              transition: 'opacity .2s, transform .2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, position: 'relative', zIndex: 1,
            }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 28, fontSize: '.95rem', fontWeight: 600, color: '#64748b', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          {isLogin ? "Don't have an account? " : "Already have an account? "}
          <button onClick={toggleMode} className="zoka-btn" style={{ background: 'none', border: 'none', color: '#10b981', fontWeight: 900, cursor: 'pointer', fontSize: '.95rem', padding: '4px 0', position: 'relative' }}>
            {isLogin ? 'Sign Up' : 'Sign In'}
          </button>
        </div>

        <div style={{ marginTop: 28, padding: '16px 18px', background: 'rgba(255,255,255,.02)', border: '1px solid #151b26', borderRadius: 14, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(16,185,129,.06)', border: '1px solid rgba(16,185,129,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Shield size={18} style={{ color: '#10b981' }} />
          </div>
          <div>
            <p style={{ textAlign: 'left', margin: 0, fontSize: '.82rem', fontWeight: 700, color: '#64748b', lineHeight: 1.6, marginBottom: 4 }}>Secure authentication powered by Firebase</p>
            <p style={{ textAlign: 'left', margin: 0, fontSize: '.78rem', fontWeight: 600, color: '#64748b', lineHeight: 1.5, opacity: .7 }}>Configure in <code style={{ background: 'rgba(255,255,255,.06)', padding: '2px 8px', borderRadius: 6, fontSize: '.75rem', fontWeight: 700 }}>.env</code> to enable live mode.</p>
          </div>
        </div>
      </div>
    </div>
