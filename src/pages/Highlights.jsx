...
export default function Highlights() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingPost) {
        await updateDoc(doc(db, 'news_posts', editingPost.id), { ...formData, updatedAt: serverTimestamp() });
      } else {
  ...
  const generateJsonLd = (post) => {
    ...
    return {
      ...
      "@type": "NewsArticle",
      "headline": post.title,
      "image": [post.imageUrl || "https://zokascore.xyz/logo.png"],
      "datePublished": post.createdAt?.toMillis ? new Date(post.createdAt.toMillis()).toISOString() : new Date().toISOString(),
      "dateModified": post.updatedAt?.toMillis ? new Date(post.updatedAt.toMillis()).toISOString() : new Date().toISOString(),
      "author": [{ "@type": "Person", "name": post.authorName || "ZOKASCORE Admin" }],
      "publisher": { "@type": "Organization", "name": "ZOKASCORE", "logo": { "@type": "ImageObject", "url": "https://zokascore.xyz/logo.png" } },
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: 'var(--nh-surface)', borderBottom: '1px solid var(--nh-border)', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {activePost && (
              <button onClick={() => navigate('/highlights')} className="nh-btn" style={{ background: 'var(--nh-surface-hover)', color: 'var(--nh-text)', width: 36, height: 36, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <ArrowLeft size={18} />
              </button>
            )}
  ...
  return (
    <div className="nh-enter" style={{ animationDelay: `${index * 50}ms`, background: 'var(--nh-surface)', borderRadius: 16, border: '1px solid var(--nh-border)', overflow: 'hidden', boxShadow: 'var(--nh-shadow)' }}>
      <div style={{ padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {isAdmin && (
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => onEdit(post)} className="nh-btn" style={{ background: 'var(--nh-surface-hover)', color: 'var(--nh-text-muted)', padding: 6, borderRadius: 8, border: '1px solid var(--nh-border)' }}><Pencil size={14} /></button>
            <button onClick={() => onDelete(post.id)} className="nh-btn" style={{ background: 'var(--nh-danger-bg)', color: 'var(--nh-danger)', padding: 6, borderRadius: 8, border: '1px solid var(--nh-danger-bg)' }}><Trash2 size={14} /></button>
          </div>
        )}
      </div>

      <div style={{ padding: '0 16px 12px', cursor: 'pointer' }} onClick={() => onExpand(post.id)}>
      ...
      {post.imageUrl && <img src={post.imageUrl} alt={post.title} style={{ width: '100%', maxHeight: 400, objectFit: 'cover', borderBottom: '1px solid var(--nh-border)' }} loading="lazy" />}
      ...
      <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--nh-border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '.8rem', color: 'var(--nh-text-muted)', fontWeight: 600 }}><ThumbsUp size={14} fill="var(--nh-accent)" color="var(--nh-accent)" /> <span>{post.likesCount || 0}</span></div>
        <div style={{ fontSize: '.8rem', color: 'var(--nh-text-muted)', cursor: 'pointer', fontWeight: 600 }} onClick={() => setExpandedComments(p => ({ ...p, [post.id]: !p[post.id] }))}>{post.commentsCount || 0} Comments</div>
      </div>

      <div style={{ display: 'flex', padding: '4px 8px', borderTop: '1px solid var(--nh-border)' }}>
        <button onClick={() => onLike(post)} className="nh-btn nh-like-anim" key={hasLiked} style={{ flex: 1, padding: '10px', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: hasLiked ? 'var(--nh-accent)' : 'var(--nh-text-muted)', fontWeight: 600, background: 'transparent' }}>
          <Heart size={18} fill={hasLiked ? 'var(--nh-accent)' : 'none'} /> Like
        </button>
        <button onClick={() => setExpandedComments(p => ({ ...p, [post.id]: !p[post.id] }))} className="nh-btn" style={{ flex: 1, padding: '10px', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: 'var(--nh-text-muted)', fontWeight: 600, background: 'transparent' }}>
  ...
  return (
    <div className="nh-enter" style={{ background: 'var(--nh-surface)', borderRadius: 16, border: '1px solid var(--nh-border)', overflow: 'hidden', boxShadow: 'var(--nh-shadow)' }}>
      ...
      <div style={{ padding: '0 16px 12px' }}>
        <h3 style={{ margin: '0 0 8px', fontSize: '1.2rem', fontWeight: 700, lineHeight: 1.4 }}>{post.title}</h3>
        <p style={{ margin: 0, color: 'var(--nh-text-muted)', lineHeight: 1.6, fontSize: '1rem', whiteSpace: 'pre-wrap' }}>{post.body}</p>
      </div>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--nh-border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '.8rem', color: 'var(--nh-text-muted)', fontWeight: 600 }}><ThumbsUp size={14} fill="var(--nh-accent)" color="var(--nh-accent)" /> <span>{post.likesCount || 0}</span></div>
        <div style={{ fontSize: '.8rem', color: 'var(--nh-text-muted)', cursor: 'pointer', fontWeight: 600 }}>{post.commentsCount || 0} Comments</div>
      </div>
      <div style={{ display: 'flex', padding: '4px 8px', borderTop: '1px solid var(--nh-border)' }}>
        <button onClick={() => onLike(post)} className="nh-btn nh-like-anim" key={hasLiked} style={{ flex: 1, padding: '10px', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, color: hasLiked ? 'var(--nh-accent)' : 'var(--nh-text-muted)', fontWeight: 600, background: 'transparent' }}>
          <Heart size={18} fill={hasLiked ? 'var(--nh-accent)' : 'none'} /> Like
