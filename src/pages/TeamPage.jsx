...
export default function TeamPage() {
  ...
  // ★ Sequential fallback done correctly: try team fixtures, fall back on failure
  const { data: fixtures = [], isLoading: fixturesLoading } = useQuery({
    queryKey: ['team-fixtures', teamId],
    queryFn: async () => {
      try {
        const r = await footballApi.getTeamFixtures(teamId);
        if (r?.data?.length) return r.data;
      } catch (e) {
        if (e?.response?.status !== 404 && !e?.notFound) throw e;
      }
      // fallback only reached when team fixtures were empty/404
      const alt = await footballApi.getResults({ teamId, limit: 20 }).catch(() => ({ data: [] }));
      return alt?.data || [];
    },
    enabled: !!teamId,
  ...
  const teamName = useMemo(
    () =>
      teamData?.name ||
      (slug ? slug.replace(/-/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase()) : 'Team'),
    [teamData, slug]
  );
    ...
    <div className="zoka-page">
      ...
      <div className="zoka-wrap">
          ...
          <div className="flex-center gap-16 mb-16">
            ...
            <div>
              ...
              <div className="team-meta">
                ...
                {teamData?.venue && (
                  <span className="flex-center gap-4"><Building2 size={14} /> {teamData.venue}</span>
                )}
              </div>
            </div>
          </div>
          <button onClick={openAI} className="btn btn-primary w-full flex-center gap-8 ai-gradient">
        ...
        <div className="glass-card p-20 mb-24">
          {fixturesLoading ? (
            <p className="empty-loading">Loading schedule...</p>
          ) : fixtures.length === 0 ? (
            <p className="empty-loading">No upcoming matches found.</p>
          ) : (
            <div className="flex-col gap-8">
              {fixtures.map((m) => {
                const home = m.homeName || m.homeTeamName || m.homeTeam?.name || 'Home';
                const away = m.awayName || m.awayTeamName || m.awayTeam?.name || 'Away';
                return (
                  <Link to={buildMatchRoute(m.id, home, away)} key={m.id} className="match-row-link">
                    <div className="match-row-teams">
                      <span>{home}</span>
                      <span className="vs muted">vs</span>
                      <span>{away}</span>
                    </div>
                    <span className="text-xs muted flex-center gap-4">
                      <Calendar size={12} />
                      {m.date ? new Date(m.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : 'TBD'}{' '}
                      <strong className="primary">{m.kickoff || ''}</strong>
                    </span>
                  </Link>
                );
              })}
        ...
        <div className="glass-card p-20 mb-24">
          {historicalResults.length > 0 ? (
            <div className="flex-col gap-8">
              {historicalResults.slice(0, 10).map((m) => {
                const home = m.homeName || m.homeTeamName || m.homeTeam?.name || 'Home';
                const away = m.awayName || m.awayTeamName || m.awayTeam?.name || 'Away';
                return (
                  <Link to={buildMatchRoute(m.id, home, away)} key={m.id} className="match-row-link">
                    <div className="match-row-teams">
                      <span>{home}</span>
                      <span className="primary bold mx-4">{m.homeScore} - {m.awayScore}</span>
                      <span>{away}</span>
                    </div>
        ...
        {teamData?.leagueId && (
          <div className="glass-card p-20">
            <h3 className="label-xs mb-12 flex-center gap-4"><Trophy size={12} /> League Directory</h3>
            <Link
              to={buildLeagueRoute(teamData.leagueId, teamData.leagueName || 'League')}
              className="btn btn-ghost w-full flex-center gap-8"
            >
              View {teamData.leagueName} Standings & Fixtures <ArrowLeft size={14} className="rotate-180" />
            </Link>
          </div>
        )}
      </div>
