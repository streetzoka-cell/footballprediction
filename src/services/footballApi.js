...
export const footballApi = {
  ...
  getH2H: (team1Id, team2Id) => monitorApiCall(`GET /match/h2h`, () => fetchJSON(`${API_BASE}/match/h2h?team1=${team1Id}&team2=${team2Id}`)),
  getVideos: () => monitorApiCall('GET /data/videos.json', () => fetchJSON(`${STATIC_BASE}/videos.json`)),
  getHealth: () => fetchJSON(`${API_BASE}/health`),

  // ★ PHASE 8: Added Historical Results Query
  getResults: (params = {}) =>
    monitorApiCall('GET /results', () => {
      const query = new URLSearchParams(params).toString();
      return fetchJSON(`${API_BASE}/results${query ? '?' + query : ''}`);
    }),

  getFeatured: (date) => monitorApiCall(`GET /featured?date=${date}`, () => fetchJSON(`${API_BASE}/featured?date=${date}`)),
  getZokaPicks: (date) => monitorApiCall(`GET /zoka-picks?date=${date}`, () => fetchJSON(`${API_BASE}/zoka-picks?date=${date}`)),
  ...
  adminZokaGetHistory: (days = 7) => monitorApiCall(`GET /zoka-picks/history?days=${days}`, () => authFetchJSON(`${API_BASE}/zoka-picks/history?days=${days}`)),
  
  adminLeaderboardRebuild: (period, dateStr) => monitorApiCall(`POST /admin/leaderboards/rebuild/${period}`, () => authFetchJSON(`${API_BASE}/admin/leaderboards/rebuild/${period}`, { method: 'POST', body: JSON.stringify({ dateStr }) })),
  
  getUserPredictions: (date) => monitorApiCall(`GET /predictions/user?date=${date}`, () => authFetchJSON(`${API_BASE}/predictions/user?date=${date}`)),
  ...
  getDailyLeaderboard: (date) => monitorApiCall(`GET /leaderboard/daily/${date}`, () => fetchJSON(`${API_BASE}/leaderboard/daily/${date}`)),
  getLeaderboardSummary: (period) => monitorApiCall(`GET /leaderboard/summary/${period}`, () => fetchJSON(`${API_BASE}/leaderboard/summary/${period}`)),
