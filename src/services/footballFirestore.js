...
import { footballDb } from '../utils/firebase';
...
export async function getFixturesForDates(dateStrs, { force = false } = {}) {
  ...
  if (uncached.length > 0) {
    await Promise.all(uncached.map(d =>
...
export function subscribeLive(callback, onError) {
  ...
  _liveUnsub = onSnapshot(
    ...
    (snap) => {
      const raw = snap.data();
      callback({ data: raw?.matches || [], lastUpdated: tsToISO(raw?.updatedAt) });
    },
    (err) => {
...
export async function getLive() {
  ...
  const raw = snap.data();
  const result = { data: raw?.matches || [], lastUpdated: tsToISO(raw?.updatedAt) };
  memCache.set(cacheKey, result);
  return result;
}
...
export async function getCompetitions({ force = false } = {}) {
  ...
  const raw = snap.data();
  const result = { data: raw?.competitions || [], lastUpdated: tsToISO(raw?.updatedAt) };
  memCache.set(cacheKey, result);
  setLocal(cacheKey, result);
  return result;
}
...
export async function getStandings(code, { force = false } = {}) {
  ...
  const raw = snap.data();
  const result = { data: raw.standings || [], lastUpdated: tsToISO(raw.updatedAt) };
  memCache.set(cacheKey, result);
  setLocal(cacheKey, result);
  return result;
}
...
export async function getTeams(code, { force = false } = {}) {
  ...
  if (!snap.exists()) return { data: null, lastUpdated: null };
  const raw = snap.data();
  const result = { data: raw.teams || [], lastUpdated: tsToISO(raw.updatedAt) };
  memCache.set(cacheKey, result);
  setLocal(cacheKey, result);
  return result;
}
