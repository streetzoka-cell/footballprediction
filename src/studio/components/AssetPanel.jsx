    ...
    <div className="studio-panel-container">
      ...
      <div>
        ...
        <div className="studio-grid-4">
          {BACKGROUNDS.map((bg, i) => (
            <button key={i} onClick={() => setBackground(bg)} className="studio-bg-btn" style={{ background: bg.fill, color: bg.name === 'White' || bg.name === 'Gold' ? '#000' : '#fff' }}>{bg.name}</button>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="studio-grid-2">
          {SFX.map((sfx, i) => (
            <button key={i} onClick={() => addSFX(sfx)} className="studio-sfx-btn">🎵 {sfx.name}</button>
          ))}
        </div>
      </div>
    </div>
