...
const wrapText = (ctx, text, mw, fontString) => {
  ...
  for (let i = 1; i < words.length; i++) {
    ...
    if (ctx.measureText(testLine).width < mw) currentLine = testLine;
...
const getAvatarPath = (shape, r) => {
  const key = `${shape}_${r}`;
  if (avatarPathCache.has(key)) return avatarPathCache.get(key);
  const path = new Path2D();
  if (shape === 'square') path.rect(-r, -r, r * 2, r * 2);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const px = rad * Math.cos(angle); const py = rad * Math.sin(angle);
      if (i === 0) path.moveTo(px, py); else path.lineTo(px, py);
    }
    path.closePath();
  } else path.arc(0, 0, r, 0, Math.PI * 2);
  avatarPathCache.set(key, path);
  return path;
};
...
const FastTextArea = React.memo(({ value, onChange, ...props }) => {
  const [local, setLocal] = useState(value);
  useEffect(() => { setLocal(value); }, [value]);
  return <textarea {...props} value={local} onChange={e => setLocal(e.target.value)} onBlur={() => onChange(local)} />;
});

const FastColor = React.memo(({ value, onChange, ...props }) => {
  const [local, setLocal] = useState(value);
  useEffect(() => { setLocal(value); }, [value]);
  return <input type="color" {...props} value={local} onChange={e => setLocal(e.target.value)} onBlur={() => onChange(local)} />;
});

const FastRange = React.memo(({ value, onChange, ...props }) => {
  return <input type="range" {...props} value={value} onChange={e => onChange(parseFloat(e.target.value))} />;
});
...
const getGameState = () => {
  try { return JSON.parse(localStorage.getItem('reactor-game-state')) || { xp: 0, level: 1, achievements: [], streak: 0, lastActive: null, totalExports: 0, templatesUsed: [], effectsUsed: [], undoCount: 0 }; }
...
const AVATAR_SHAPES = [{ id: 'circle', name: 'Circle' }, { id: 'square', name: 'Square' }, { id: 'hexagon', name: 'Hexagon' }, { id: 'star', name: 'Star' }];
const AVATAR_ANIMS = [{ id: 'none', name: 'None' }, { id: 'pulse', name: 'Pulse' }, { id: 'rotate', name: 'Rotate' }];
  ...
  switch (action.type) {
    ...
    case 'SET_EDITOR': {
      ...
      if (shouldTrack && (action.payload.templateId !== undefined || action.payload.videoEffect !== undefined || action.payload.filter !== undefined)) {
        return { ...state, editor: newEditor, history: { past: [...state.history.past.slice(-30), { editor: state.editor }], future: [] } };
      }
...
function ReactorStudioInner() {
  ...
  const exportProgressTextRef = useRef(null);
  ...
  const exportDurationRef = useRef(0);
  ...
  const handleToolbarScroll = useCallback(() => {
    ...
    if (!el) return;
    if (isMobile) setScrollState({ left: el.scrollLeft > 10, right: el.scrollLeft < el.scrollWidth - el.clientWidth - 10, up: false, down: false });
    else setScrollState({ left: false, right: false, up: el.scrollTop > 10, down: el.scrollTop < el.scrollHeight - el.clientHeight - 10 });
  }, [isMobile]);
  ...
  useEffect(() => {
    const handleFsChange = () => {
      ...
      if (isFs !== stateRef.current.ui.fullscreen) dispatch({ type: 'SET_UI', payload: { fullscreen: isFs } });
    };
  ...
  useEffect(() => {
    if (fixtureData) {
      dispatch({ type: 'SET_EDITOR', payload: { homeLogoUrl: fixtureData.homeLogo || '', awayLogoUrl: fixtureData.awayLogo || '', homeScore: fixtureData.score?.home ?? 0, awayScore: fixtureData.score?.away ?? 0, povCaption: `${fixtureData.minute || ''} GOAL! ${fixtureData.scorer || ''} scores! 🔥`, templateId: 'pro_aura' } });
  ...
  useEffect(() => {
    if (stateRef.current.ui.isLoadingProject) return;
    dispatch({ type: 'SET_UI', payload: { autoSaveStatus: 'saving' } });
    clearTimeout(autoSaveTimerRef.current);
    autoSaveTimerRef.current = setTimeout(() => {
      localStorage.setItem('reactor-project-state', JSON.stringify({ editor: stateRef.current.editor, timeline: stateRef.current.timeline }));
  ...
  const togglePreview = useCallback(() => {
    ...
    if (s.editor.mode === 'video' && s.media.sourceLoaded) {
      const vid = sourceVideoRef.current;
      const aClip = s.timeline.clips.find(c => c.id === s.timeline.activeClipId) || s.timeline.clips[0];
      if (!vid || !aClip) return;
      if (s.timeline.isPlaying) { vid.pause(); dispatch({ type: 'SET_TIMELINE', payload: { isPlaying: false } }); }
      else {
        if (vid.currentTime < aClip.start || vid.currentTime >= aClip.end - 0.1) vid.currentTime = aClip.start;
        vid.play(); dispatch({ type: 'SET_TIMELINE', payload: { isPlaying: true } });
      }
    } else {
      dispatch({ type: 'SET_TIMELINE', payload: { isPlaying: !s.timeline.isPlaying } });
    }
    haptic('light');
  }, []);
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'm') dispatch({ type: 'SET_EDITOR', payload: { isMuted: !stateRef.current.editor.isMuted } });
      if (e.key === 'g') dispatch({ type: 'SET_UI', payload: { showGuides: !stateRef.current.ui.showGuides } });
      if (e.key === 'e') dispatch({ type: 'SET_EDITOR', payload: { editMode: !stateRef.current.editor.editMode } });
      if (e.key === 'f') toggleFullscreen();
      if (e.key === '?') dispatch({ type: 'SET_UI', payload: { showShortcuts: !stateRef.current.ui.showShortcuts } });
      if (e.key === 'Escape') dispatch({ type: 'SET_UI', payload: { activePanel: null, showShortcuts: false } });
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [togglePreview, toggleFullscreen]);

  const handleImport = useCallback(async (e, type) => {
    ...
    if (file) {
      ...
      if (type === 'video') {
        ...
        sourceVideoRef.current.muted = stateRef.current.editor.isMuted;
        sourceVideoRef.current.onloadedmetadata = () => {
          const dur = sourceVideoRef.current.duration;
          ...
          dispatch({ type: 'SET_TIMELINE', payload: { duration: dur, clips: newClips, activeClipId: newClips[0].id, isPlaying: true } });
          dispatch({ type: 'SET_MEDIA', payload: { sourceLoaded: true } });
        ...
        brollVideoRef.current.onloadedmetadata = () => { brollVideoRef.current.play(); dispatch({ type: 'SET_MEDIA', payload: { brollLoaded: true, cameraOn: false } }); unlockAchievement('pip_pro'); addToast('B-Roll added!', 'success'); };
      } else if (type === 'image') {
  ...
  const handleImportImages = useCallback(async (e) => {
    ...
    const dur = imageUrls.length * stateRef.current.editor.slideshowSpeed;
    dispatch({ type: 'SET_SLIDESHOW', payload: { images: imageUrls, duration: dur } });
    dispatch({ type: 'SET_EDITOR', payload: { mode: 'slideshow' } });
    dispatch({ type: 'SET_TIMELINE', payload: { duration: dur, clips: [{ id: 'clip1', start: 0, end: dur }], isPlaying: true } });
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      if (!stateRef.current.media.sourceLoaded) {
  ...
  const drawVerifiedBadge = useCallback((ctx, x, y, s) => {
  ...
  const drawFrame = useCallback(() => {
    ...
    const s = stateRef.current;
    const { media, editor, slideshow, timeline, ui } = s;
    const aTemplate = templateMap[editor.templateId] || TEMPLATES[0];
    const aClip = timeline.clips.find(c => c.id === timeline.activeClipId) || timeline.clips[0];
    ...
    const bgColor = editor.bgColor || aTemplate.bg;
    ...
    if (editor.mode === 'slideshow' && slideshow.images.length > 0) {
      const imgDur = editor.slideshowSpeed; const totalDur = slideshow.images.length * imgDur; const loopedTime = cTime % totalDur;
      ...
      if (aPiPVid && aPiPVid.videoWidth) {
        ctx.save(); ctx.scale(-1, 1); ctx.translate(-W, 0); drawCover(ctx, aPiPVid, 0, 0, W, H); ctx.restore();
      }
      ...
      if (ui.layers.video) {
        const v = aTemplate.video; 
        const aProg = aClip ? Math.min((cTime - aClip.start) / (aClip.end - aClip.start), 1) : 0;
        
        ctx.save(); 
        if (editor.videoEffect === 'zoom_in') { const sc = 1 + aProg * 0.3; ctx.translate(v.x + v.w / 2, v.y + v.h / 2); ctx.scale(sc, sc); ctx.translate(-(v.x + v.w / 2), -(v.y + v.h / 2)); }
        else if (editor.videoEffect === 'shake') ctx.translate((Math.random() - 0.5) * 15, (Math.random() - 0.5) * 15);
        else if (editor.videoEffect === 'pulse') { const sc = 1 + Math.sin(cTime * 8) * 0.04; ctx.translate(v.x + v.w / 2, v.y + v.h / 2); ctx.scale(sc, sc); ctx.translate(-(v.x + v.w / 2), -(v.y + v.h / 2)); }
        else if (editor.videoEffect === 'ken_burns') { const sc = 1 + aProg * 0.15; const tx = aProg * 30; ctx.translate(v.x + v.w / 2 - tx, v.y + v.h / 2); ctx.scale(sc, sc); ctx.translate(-(v.x + v.w / 2), -(v.y + v.h / 2)); }
        ...
        if (editor.swapPip && aPiPVid && showPiP) {
          ...
          drawCover(ctx, aPiPVid, v.x, v.y, v.w, v.h, mainCrop); ctx.restore();
        } else {
        ...
        if (editor.videoEffect === 'flash' && cTime < (aClip?.start || 0) + 0.5) { ctx.fillStyle = `rgba(255,255,255,${1 - (cTime - (aClip?.start || 0)) * 2})`; ctx.fillRect(0, 0, W, H); }
        if (editor.fadeIn && cTime < (aClip?.start || 0) + 1) { ctx.fillStyle = `rgba(0,0,0,${1 - (cTime - (aClip?.start || 0))})`; ctx.fillRect(0, 0, W, H); }
      }

      if (aPiPVid && showPiP) {
        const baseP = dragStateRef.current?.target === 'pip' ? dragStateRef.current.tempPos : editor.pipPos;
        const scale = editor.pipScale || 1.0; const pW = Math.round(baseP.w * scale); const pH = Math.round(baseP.h * scale); 
        ...
        if (editor.swapPip) {
          ctx.save(); roundRectPath(ctx, pX, pY, pW, pH, radius); ctx.clip(); drawCover(ctx, sourceVideoRef.current, pX, pY, pW, pH); ctx.restore();
        } else {
          ctx.save();
    ...
    const aProg = aClip ? Math.min((cTime - aClip.start) / 2, 1) : 0;
    if (ui.layers.gradients) {
      if (aTemplate.topGradient) { const g = ctx.createLinearGradient(0, 0, 0, aTemplate.topGradient); g.addColorStop(0, 'rgba(0,0,0,0.8)'); g.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = g; ctx.fillRect(0, 0, W, aTemplate.topGradient); }
      if (aTemplate.bottomGradient) { const g = ctx.createLinearGradient(0, H - aTemplate.bottomGradient, 0, H); g.addColorStop(0, 'rgba(0,0,0,0)'); g.addColorStop(1, 'rgba(0,0,0,0.9)'); ctx.fillStyle = g; ctx.fillRect(0, H - aTemplate.bottomGradient, W, aTemplate.bottomGradient); }
    }
    if (aTemplate.header) { const h = aTemplate.header; ctx.fillStyle = h.bg; ctx.fillRect(0, 0, W, h.h); ctx.fillStyle = '#fff'; ctx.font = `bold ${h.size}px ${font.name}`; ctx.textAlign = 'center'; ctx.fillText(h.text, W / 2, h.y); }
    if (aTemplate.ticker) { const t = aTemplate.ticker; ctx.fillStyle = t.bg; ctx.fillRect(0, t.y, W, t.h); ctx.fillStyle = '#fff'; ctx.font = `bold ${t.size}px ${font.name}`; ctx.textAlign = 'left'; wrapText(ctx, editor.povCaption, W - 40, `bold ${t.size}px ${font.name}`).forEach((l, i) => ctx.fillText(l, 20, t.y + 40 + (i * 36))); }

    if (ui.layers.caption && aTemplate.caption && !aTemplate.ticker) {
      const c = aTemplate.caption; ctx.fillStyle = editor.captionColor || c.color || '#fff';
      const cS = editor.captionSize ? parseInt(editor.captionSize) : c.size;
      const fontStr = `${font.weight} ${cS}px ${font.name}`;
      ctx.font = fontStr; ctx.textAlign = c.center ? 'center' : (c.align || 'left');
      ...
      let yP = c.y + yO; wrapText(ctx, dC, c.maxW, fontStr).forEach(l => { ctx.fillText(l, c.x, yP); yP += cS + 8; });
      ctx.globalAlpha = 1;
    }

    const p = (aTemplate.isCustom || editor.editMode) ? (dragStateRef.current?.target === 'profile' ? dragStateRef.current.tempPos : editor.profilePos) : aTemplate.profile;
    if (profileImgRef.current.src && p && ui.layers.profile) {
      ...
      ctx.translate(p.x, p.y); ctx.rotate(animRot); ctx.scale(animScale, animScale);
      const shapePath = getAvatarPath(editor.avatarShape || 'circle', p.r);
      ctx.clip(shapePath);
      ctx.drawImage(profileImgRef.current, -p.r, -p.r, p.r * 2, p.r * 2);
      ctx.restore();
      if (p.ring) {
        ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(animRot); ctx.scale(animScale, animScale);
        ctx.strokeStyle = p.ring === 'accent' ? editor.accentColor : p.ring; ctx.lineWidth = 4;
        ctx.stroke(getAvatarPath(editor.avatarShape || 'circle', p.r + 1));
        ctx.restore();
      }
    }

    if (aTemplate.nameEl && aTemplate.handleEl) {
      const n = aTemplate.nameEl, hd = aTemplate.handleEl;
      ...
      const nx = (aTemplate.isCustom || editor.editMode) ? p.x + p.r + 12 : n.x;
      const ny = (aTemplate.isCustom || editor.editMode) ? p.y + 10 : n.y;
    ...
    } else if (aTemplate.username || editor.editMode) {
      const u = aTemplate.username || { size: 28, center: true };
      const ux = (aTemplate.isCustom || editor.editMode) ? p.x : u.x;
      const uy = (aTemplate.isCustom || editor.editMode) ? p.y + p.r + 30 : u.y;
      ctx.textAlign = (aTemplate.isCustom || editor.editMode) ? 'center' : (u.center ? 'center' : (u.align || 'left'));
      ...
      if (editor.showVerified) { let nW = ctx.measureText(editor.username).width; let bX = (aTemplate.isCustom || editor.editMode || u.center) ? ux + nW / 2 + 16 : ux + nW + 16; drawVerifiedBadge(ctx, bX, uy - uS / 2 + 2, uS / 2.5); }
    }

    const activeStickers = dragStateRef.current?.target?.startsWith('sticker_') ? dragStateRef.current.tempStickers : editor.stickers;
    if (ui.layers.stickers && activeStickers && activeStickers.length > 0) {
      ctx.globalAlpha = editor.stickerOpacity;
      activeStickers.forEach(st => {
        ctx.font = `${st.size || 60}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillText(st.emoji, st.x, st.y);
        if (st.id === editor.selectedStickerId) {
          ctx.globalAlpha = 1; ctx.strokeStyle = '#10b981'; ctx.lineWidth = 2; ctx.setLineDash([6, 6]);
          ctx.strokeRect(st.x - st.size/2, st.y - st.size/2, st.size, st.size); ctx.setLineDash([]);
          ctx.fillStyle = '#10b981'; ctx.beginPath(); ctx.arc(st.x + st.size/2, st.y + st.size/2, 16, 0, Math.PI * 2); ctx.fill();
    ...
    if (editor.introEnabled && aClip && editor.mode !== 'camera') {
      const introDur = 3.0; const introP = Math.min((cTime - aClip.start) / introDur, 1.0);
      if (introP < 1.0) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const isPlaying = s.timeline.isPlaying;
      const isCamera = s.media.cameraOn;
      ...
      if (isPlaying && (s.editor.mode === 'slideshow' || s.editor.mode === 'camera')) {
         currentTimeRef.current += delta * s.editor.playbackRate;
         if (s.editor.mode === 'slideshow' && currentTimeRef.current >= s.slideshow.duration) currentTimeRef.current = 0;
      } else if (s.editor.mode === 'video' && s.media.sourceLoaded && sourceVideoRef.current) {
         currentTimeRef.current = sourceVideoRef.current.currentTime;
      }

      if (isDirtyRef.current || isPlaying || isCamera || isExporting || isDragging || isAnimatedEffect) {
        drawFrame();
        ...
        if (isExporting && s.editor.mode === 'video') {
           const vid = sourceVideoRef.current;
           if (vid && exportDurationRef.current > 0) {
              let compDur = 0;
              for(let i=0; i<currentExportClipRef.current; i++) compDur += (s.timeline.clips[i].end - s.timeline.clips[i].start);
              const c = s.timeline.clips[currentExportClipRef.current];
              const currProg = c ? Math.max(0, vid.currentTime - c.start) : 0;
              const prog = Math.min(((compDur + currProg) / exportDurationRef.current) * 100, 99);
              if (exportProgressFillRef.current) exportProgressFillRef.current.style.width = `${prog}%`;
              if (exportProgressTextRef.current) exportProgressTextRef.current.innerText = `${Math.round(prog)}%`;
           }
        }
      }
  ...
  const handlePointerDown = useCallback((e) => {
    ...
    const s = stateRef.current;
    const selSticker = s.editor.stickers?.find(st => st.id === s.editor.selectedStickerId);
    if (selSticker) {
      ...
      if (Math.hypot(x - handleX, y - handleY) < 30) {
        dragStateRef.current = { target: 'sticker_resize', id: selSticker.id, tempStickers: [...s.editor.stickers] };
        return;
      }
    }
    const clickedSticker = s.editor.stickers?.find(st => Math.abs(x - st.x) < st.size/2 && Math.abs(y - st.y) < st.size/2);
    if (clickedSticker) {
      dispatch({ type: 'SET_EDITOR', payload: { selectedStickerId: clickedSticker.id } });
      dragStateRef.current = { target: 'sticker_move', id: clickedSticker.id, offsetX: x - clickedSticker.x, offsetY: y - clickedSticker.y, tempStickers: [...s.editor.stickers] };
      return;
    }
    if (s.editor.selectedStickerId) dispatch({ type: 'SET_EDITOR', payload: { selectedStickerId: null } });
    
    const aTemplate = templateMap[s.editor.templateId] || TEMPLATES[0];
    if (!s.editor.editMode && !aTemplate.isCustom && !s.media.brollLoaded && !s.media.cameraOn) return;
    if ((aTemplate.isCustom || s.editor.editMode) && s.media.profileSrc) { 
      const pPos = s.editor.profilePos;
      if (Math.hypot(x - pPos.x, y - pPos.y) <= pPos.r) { 
        dragStateRef.current = { target: 'profile', offsetX: x - pPos.x, offsetY: y - pPos.y, tempPos: { ...pPos } }; return; 
      } 
    }
    if (s.media.brollLoaded || s.media.cameraOn) { 
      const baseP = s.editor.pipPos; const scale = s.editor.pipScale || 1.0; 
      const pW = Math.round(baseP.w * scale); const pH = Math.round(baseP.h * scale); 
      const pX = Math.round(baseP.x + (baseP.w - pW) / 2); const pY = Math.round(baseP.y + (baseP.h - pH) / 2); 
      if (x >= pX && x <= pX + pW && y >= pY && y <= pY + pH) { dragStateRef.current = { target: 'pip', offsetX: x - pX, offsetY: y - pY, tempPos: { ...baseP } }; } 
    }
  }, [getCanvasCoords, templateMap]);
  ...
  const handlePointerMove = useCallback((e) => {
    ...
    if (ds.target === 'sticker_move' || ds.target === 'sticker_resize') {
      const idx = ds.tempStickers.findIndex(st => st.id === ds.id);
      if (idx !== -1) {
        if (ds.target === 'sticker_move') ds.tempStickers[idx] = { ...ds.tempStickers[idx], x: x - ds.offsetX, y: y - ds.offsetY };
        else { const newSize = Math.max(30, Math.min(400, Math.hypot(x - ds.tempStickers[idx].x, y - ds.tempStickers[idx].y) * 1.5)); ds.tempStickers[idx] = { ...ds.tempStickers[idx], size: newSize }; }
      }
    } else if (ds.target === 'pip') { 
      const baseP = ds.tempPos; const scale = stateRef.current.editor.pipScale || 1.0; const pW = Math.round(baseP.w * scale); const pH = Math.round(baseP.h * scale); 
      let nX = Math.max(0, Math.min(x - ds.offsetX, canvasRef.current.width - pW)); 
      let nY = Math.max(0, Math.min(y - ds.offsetY, canvasRef.current.height - pH)); 
      const sx = [0, canvasRef.current.width/2 - pW/2, canvasRef.current.width - pW]; 
      sx.forEach(pt => { if (Math.abs(nX - pt) < 20) nX = pt; }); 
      ds.tempPos.x = nX - (baseP.w - pW) / 2; ds.tempPos.y = nY - (baseP.h - pH) / 2; 
    } else if (ds.target === 'profile') { 
      let nX = Math.max(ds.tempPos.r, Math.min(x - ds.offsetX, canvasRef.current.width - ds.tempPos.r)); 
      let nY = Math.max(ds.tempPos.r, Math.min(y - ds.offsetY, canvasRef.current.height - ds.tempPos.r)); 
      ds.tempPos.x = nX; ds.tempPos.y = nY; 
    }
  ...
  const handlePointerUp = useCallback(() => {
    if (dragStateRef.current) {
      const ds = dragStateRef.current;
      if (ds.target === 'pip') dispatch({ type: 'SET_EDITOR', payload: { pipPos: ds.tempPos } });
      else if (ds.target === 'profile') dispatch({ type: 'SET_EDITOR', payload: { profilePos: ds.tempPos } });
      else if (ds.target.startsWith('sticker_')) dispatch({ type: 'SET_EDITOR', payload: { stickers: ds.tempStickers } });
      dragStateRef.current = null;
    }
  }, []);

  const handleExportVideo = useCallback(async (preset) => {
    const { format, fps, bitrate } = preset;
    const s = stateRef.current;
    const vid = sourceVideoRef.current;
    if (!canvasRef.current || s.ui.isExporting) return;
    if (s.editor.mode === 'video' && !s.media.sourceLoaded) return;
    if (s.editor.mode === 'slideshow' && s.slideshow.images.length === 0) return;
    ...
    if (s.editor.mode === 'video' && s.media.sourceLoaded) {
      exportDuration = s.timeline.clips.reduce((sum, c) => sum + (c.end - c.start), 0);
      currentExportClipRef.current = 0;
      vid.pause(); vid.currentTime = s.timeline.clips[0].start;
      await new Promise(r => setTimeout(r, 200));
      vid.muted = false; vid.volume = 0;
    } else if (s.editor.mode === 'camera' && s.media.cameraOn) {
      exportDuration = 30;
    } else {
      exportDuration = s.slideshow.duration;
      currentTimeRef.current = 0;
      dispatch({ type: 'SET_TIMELINE', payload: { isPlaying: true } });
    }

    exportDurationRef.current = exportDuration;
    ...
    try {
      ...
      if (vid && vid.captureStream && s.media.sourceLoaded) { try { const vS = vid.captureStream(); if (vS.getAudioTracks().length > 0) { aC.createMediaStreamSource(vS).connect(aD); } } catch { } }
      if (audioRef.current.src) { try { audioRef.current.play(); const aS = audioRef.current.captureStream ? audioRef.current.captureStream() : audioRef.current.mozCaptureStream(); if (aS.getAudioTracks().length > 0) { aC.createMediaStreamSource(aS).connect(aD); } } catch { } }
    ...
    r.onstop = async () => {
      ...
      if (vid) { vid.pause(); vid.muted = stateRef.current.editor.isMuted; vid.volume = 1; }
    ...
    r.start(100);
    if (s.editor.mode === 'video' && s.media.sourceLoaded) { try { await vid.play(); } catch { } }
    else if (s.editor.mode === 'camera') { dispatch({ type: 'SET_TIMELINE', payload: { isPlaying: true } }); }
  }, [addToast, unlockAchievement]);
  ...
  const applyTemplate = useCallback((id) => {
    ...
    if (!t) return;
    dispatch({ type: 'SET_EDITOR', payload: { templateId: id, profilePos: t.profile ? { x: t.profile.x, y: t.profile.y, r: t.profile.r } : stateRef.current.editor.profilePos, pipPos: getPipPosForTemplate(t), pipScale: 1.0 } });
    dispatch({ type: 'SET_UI', payload: { activePanel: null } });
    const nR = [id, ...stateRef.current.ui.recents.filter(r => r !== id)].slice(0, 5);
  ...
  }, [templateMap, addToast, unlockAchievement]);

  const toggleFavorite = useCallback((id) => {
    const n = stateRef.current.ui.favorites.includes(id) ? stateRef.current.ui.favorites.filter(x => x !== id) : [...stateRef.current.ui.favorites, id];
  ...
  const addSticker = useCallback((sticker) => {
    const ratios = { '9:16': { w: 720, h: 1280 }, '1:1': { w: 1080, h: 1080 }, '4:5': { w: 864, h: 1080 } };
    const dims = ratios[stateRef.current.editor.canvasRatio] || ratios['9:16'];
    const newSticker = { ...sticker, x: dims.w / 2, y: dims.h / 2, size: 100, id: Date.now() };
    dispatch({ type: 'SET_EDITOR', payload: { stickers: [...(stateRef.current.editor.stickers || []), newSticker], selectedStickerId: newSticker.id } });
    haptic('light');
  }, []);
  ...
  const renderToolButtons = useMemo(() => {
    const tools = [
      ...
      <div className="rs-main">
        <div className="rs-canvas-area">
          <div className="rs-canvas-wrap" data-ratio={editor.canvasRatio}
            ...
            {ui.isExporting && <div className="rs-canvas-exporting"><Loader size={12} className="rs-spin" /> EXPORTING...</div>}
      ...
      {(media.sourceLoaded || slideshow.images.length > 0 || media.cameraOn) && (
        <div className="rs-bottom-controls">
          ...
          <div className="rs-time-display">
            <span ref={timeCurrentRef} id="rs-time-current">0.0s</span>
            <span id="rs-time-duration">{timeline.duration.toFixed(1)}s</span>
          </div>
          <div className="rs-timeline-track">
            ...
            <div ref={playheadRef} id="rs-playhead" className="rs-timeline-playhead" style={{ left: `${(currentTimeRef.current / (timeline.duration || 1)) * 100}%` }}></div>
          </div>
        </div>
      )}
          ...
          {ui.activePanel === 'templates' && (
            <div>
              ...
              <div className="rs-search-box">
                <Search size={14} />
                <FastInput type="text" value={ui.searchQuery} onChange={(val) => dispatch({ type: 'SET_UI', payload: { searchQuery: val } })} placeholder="Search templates..." className="rs-search-input" />
              </div>
            ...
            <div>
              ...
              <div className="rs-panel-box">
                ...
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <FastColor value={editor.bgColor || '#000000'} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { bgColor: val } })} className="rs-input-color" />
              ...
              {media.sourceLoaded && editor.mode === 'video' && (
                <div className="rs-panel-box">
                  ...
                  <FastRange min="1" max="4" step="0.1" value={editor.videoZoom} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { videoZoom: val }, trackHistory: false })} className="rs-range" />
                  <label className="rs-label">Pan X: {editor.videoPanX.toFixed(1)}</label>
                  <FastRange min="-1" max="1" step="0.1" value={editor.videoPanX} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { videoPanX: val }, trackHistory: false })} className="rs-range" />
                  <label className="rs-label">Pan Y: {editor.videoPanY.toFixed(1)}</label>
                  <FastRange min="-1" max="1" step="0.1" value={editor.videoPanY} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { videoPanY: val }, trackHistory: false })} className="rs-range" />
              ...
              {(media.brollLoaded || media.cameraOn) && (editor.mode === 'video' || editor.mode === 'camera') && (
                <div className="rs-panel-box">
                  ...
                  <FastRange min="0.5" max="2" step="0.05" value={editor.pipScale || 1} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { pipScale: val }, trackHistory: false })} className="rs-range" />
          ...
          {ui.activePanel === 'text' && (
            <div>
              ...
              <div className="rs-panel-box">
                ...
                <FastInput type="text" value={editor.displayName} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { displayName: val }, trackHistory: false })} placeholder="Display Name" className="rs-input" />
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '8px' }}>
                  <FastColor value={editor.nameColor} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { nameColor: val } })} className="rs-input" style={{ width: '40px', padding: '2px', height: '36px' }} />
                  <FastInput type="number" value={editor.nameSize || ''} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { nameSize: val ? parseInt(val) : null } })} placeholder="Size" className="rs-input" style={{ width: '80px' }} />
                ...
                <FastInput type="text" value={editor.username} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { username: val }, trackHistory: false })} placeholder="@username" className="rs-input" />
                <FastTextArea value={editor.povCaption} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { povCaption: val }, trackHistory: false })} placeholder="Caption" className="rs-input" style={{ height: '60px', resize: 'none', marginBottom: '8px' }} />
                <div style={{ display: 'flex', gap: '8px' }}>
                  <FastColor value={editor.captionColor} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { captionColor: val } })} className="rs-input" style={{ width: '40px', padding: '2px', height: '36px' }} />
                  <FastInput type="number" value={editor.captionSize || ''} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { captionSize: val ? parseInt(val) : null } })} placeholder="Size" className="rs-input" style={{ width: '80px' }} />
                </div>
              </div>
              ...
              <div className="rs-panel-box">
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '8px' }}>
                  ...
                  <FastColor value={editor.accentColor} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { accentColor: val } })} style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'none', border: '2px solid #151b26', cursor: 'pointer', padding: 0 }} />
                </div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <div className="rs-panel-box">
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                  <FastInput type="text" value={editor.homeLogoUrl} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { homeLogoUrl: val } })} placeholder="Home Logo URL" className="rs-input" />
                  <FastInput type="number" value={editor.homeScore} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { homeScore: parseInt(val) || 0 } })} className="rs-input" style={{ width: '50px', flex: 'none' }} />
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <FastInput type="text" value={editor.awayLogoUrl} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { awayLogoUrl: val } })} placeholder="Away Logo URL" className="rs-input" />
                  <FastInput type="number" value={editor.awayScore} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { awayScore: parseInt(val) || 0 } })} className="rs-input" style={{ width: '50px', flex: 'none' }} />
                </div>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              {editor.stickers && editor.stickers.length > 0 && (
                <div className="rs-panel-box">
                  ...
                  <FastRange min="0.1" max="1" step="0.1" value={editor.stickerOpacity} onChange={(val) => dispatch({ type: 'SET_EDITOR', payload: { stickerOpacity: val } })} className="rs-range" />
      ...
      {showOnboarding && (
        <div className="rs-modal-overlay">
          <div className="rs-modal rs-onboarding">
            ...
            <p>Create viral short-form content in second. Import a video, pick a template, add effects, and export in 1080p.</p>
