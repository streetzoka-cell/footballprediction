...
import { 
  ...
  Play, Pause, Loader, Trash2, Move, Film, AppWindow, Palette
} from 'lucide-react';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#0a0f1a', color: '#fff', overflow: 'hidden' }}>
      <div style={{ padding: '12px 16px', background: '#111827', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #1f2937', zIndex: 10, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <h1 style={{ fontSize: '18px', fontWeight: 800, margin: 0 }}>Web Showcase Studio</h1>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          {recordedUrl ? (
            <>
              <button onClick={discardRecording} style={{ ...topBtnStyle, background: '#ef4444', borderColor: '#ef4444' }}>
                <Trash2 size={16} /> Discard
              </button>
              <a href={recordedUrl} download="web_showcase.webm" style={{ ...topBtnStyle, background: '#10b981', borderColor: '#10b981', textDecoration: 'none' }}>
                <Download size={16} /> Download
              </a>
            </>
          ) : isRecording ? (
            <button onClick={stopRecording} style={{ ...topBtnStyle, background: '#ef4444', borderColor: '#ef4444' }}>
              <Square size={16} fill="#fff" /> Stop Recording
            </button>
          ) : (
            <button onClick={startRecording} disabled={!screenReady} style={{ ...topBtnStyle, background: '#10b981', borderColor: '#10b981', opacity: !screenReady ? 0.5 : 1 }}>
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px', background: '#000', minHeight: 0 }}>
          ...
          >
            ...
            {!screenReady && !recordedUrl && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#111', color: '#64748b', cursor: 'pointer', gap: '12px' }} onClick={startScreenShare}>
                <Monitor size={48} />
                <p style={{ fontWeight: 700, fontSize: '16px' }}>Click to Share Screen</p>
                <p style={{ fontSize: '12px', opacity: 0.7 }}>(Select a Chrome Tab or Entire Screen)</p>
              </div>
            )}
          ...
          <div style={panelStyle}>
            ...
            <div style={{ fontSize: '12px', color: '#cbd5e1', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                Tab/System Audio: {screenReady && screenStreamRef.current?.getAudioTracks().length ? 'Active' : 'None'}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <span style={{ fontSize: '10px', color: '#64748b' }}>To capture website audio, make sure to check "Share Tab Audio" when selecting your screen.</span>
          </div>
        </div>
      </div>
