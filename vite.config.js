...
export default defineConfig({
    ...
    VitePWA({
      ...
      workbox: {
        ...
        navigateFallbackDenylist: [
          /^\/robots\.txt$/, /^\/ads\.txt$/, /^\/zokascore-sitemap\.xml/, 
          /^\/sitemap\.xml/, /^\/sitemap\//, /^\/api\/.*/, /^\/opensearch\.xml$/, /^\/google.*/
        ],
        runtimeCaching: [
          {
          ...
          { urlPattern: /\.(?:png|jpg|jpeg|svg|gif|webp|ico|woff2)$/i, handler: 'CacheFirst', options: { cacheName: 'assets-cache', expiration: { maxEntries: 100, maxAgeSeconds: 60 * 60 * 24 * 30 } } }
        ]
      }
    })
  ],
  server: {
    port: 5173,
    proxy: { '/api': { target: 'http://localhost:3099', changeOrigin: true, secure: false } }
  },
  build: {
