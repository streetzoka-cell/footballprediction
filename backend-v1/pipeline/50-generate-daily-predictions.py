...
def get_team_intel(team_id, live_team_state, team_states_1x2):
    elo = float(live_team_state.get(team_id, {}).get("elo", 1500.0))
    recent = list(team_states_1x2.get(team_id, {}).get("recent_matches", []))[-5:]
    
    form = []
    for m in reversed(recent):
        if m["gf"] > m["ga"]: form.append("W")
        elif m["gf"] < m["ga"]: form.append("L")
        else: form.append("D")
...
def enforce_logical_consistency(markets):
    """
    Fixes mathematically impossible predictions generated by independent models.
    Prioritizes the strongest signals (0.5 and 1.5 lines) to force lower lines.
    """
    def set_pick(m_key, pick):
        if m_key in markets and "probabilities" in markets[m_key]:
            ...
            # Ensure forced picks don't display artificially low probabilities
            markets[m_key]["pick_probability"] = max(prob, 51.0)

    # 1. O/U 0.5 UNDER means exactly 0 goals (0-0). 
    # This is the strongest constraint and overrides everything.
    ...
    # 2. O/U 1.5 UNDER means at most 1 goal (1-0, 0-1, 0-0).
    if markets.get("ou_1_5", {}).get("pick") == "UNDER":
        set_pick("btts", "NO")
        set_pick("ou_2_5", "UNDER")
        set_pick("ou_3_5", "UNDER")
        # 1 goal means Over 0.5 must hit
        set_pick("ou_0_5", "OVER")
        return markets

    # 3. O/U 2.5 UNDER means at most 2 goals.
    if markets.get("ou_2_5", {}).get("pick") == "UNDER":
        set_pick("ou_3_5", "UNDER")
        # BTTS can be YES (1-1) or NO (2-0, 0-0). Do not force BTTS here.

    # 4. O/U 3.5 OVER means at least 4 goals. Forces all lower lines to OVER.
    ...
    # 5. O/U 2.5 OVER means at least 3 goals. Forces lower lines to OVER.
    if markets.get("ou_2_5", {}).get("pick") == "OVER":
        set_pick("ou_1_5", "OVER")
        set_pick("ou_0_5", "OVER")

    # 6. O/U 1.5 OVER means at least 2 goals. Forces 0.5 to OVER.
    if markets.get("ou_1_5", {}).get("pick") == "OVER":
        set_pick("ou_0_5", "OVER")

    # 7. BTTS YES means at least 2 goals (1-1). Forces 0.5 and 1.5 to OVER.
...
def run():
    ...
    for offset in (0, 1):
        ...
        for match in matches:
            ...
            match["intelData"] = {
                "home": get_team_intel(home_id, live_team_state, team_states_1x2),
                "away": get_team_intel(away_id, live_team_state, team_states_1x2),
                "h2h": {
                    "meetings": total_h2h,
                    "teamA_wins": h2h["team_a_wins"] if home_id < away_id else h2h["team_b_wins"],
                    "teamB_wins": h2h["team_b_wins"] if home_id < away_id else h2h["team_a_wins"],
                    "draws": h2h["draws"]
                }
            }
