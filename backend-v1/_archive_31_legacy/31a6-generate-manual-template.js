...
async function main() {
  ...
  // Collect all unresolved names from all categories and deduplicate
  const unresolvedMap = new Map(); // Map<eloName, item>
  
  const allCategories = ['NO_CANDIDATE', 'NEAR_MATCH', 'AMBIGUOUS_CORE', 'RESERVE_TEAM'];
  
  for (const category of allCategories) {
    for (const item of forensic[category] || []) {
      if (!unresolvedMap.has(item.name)) {
        unresolvedMap.set(item.name, {
          eloName: item.name,
  ...
  const allUnresolved = [...unresolvedMap.values()];

  // Sort by record count descending (most impactful first)
  allUnresolved.sort((a, b) => b.recordCount - a.recordCount);
  ...
  const template = {
    ...
    total_records_at_stake: allUnresolved.reduce((sum, item) => sum + item.recordCount, 0),
    aliases: []
  };
