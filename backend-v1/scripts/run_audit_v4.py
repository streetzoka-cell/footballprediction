...
# 1. Load Models
print("[Audit V4] Loading AI Engine Models...")
...
# 2. Load Datasets
print("[Audit V4] Loading datasets...")
bt_data = []
...
# ★ FIX 1: STRICT DATE FILTER BEFORE MERGE
df['date'] = pd.to_datetime(df['date'])
df = df[df['date'] >= '2024-01-01'].copy()
print(f"[Audit V4] Strictly auditing {len(df)} matches (Out-of-Sample: 2024-01-01 onwards)...")
...
REQUIRED_COLUMNS = [
    "date", "home_team", "away_team", "target.result", 
    "odds.home", "odds.draw", "odds.away"
]
missing = [c for c in REQUIRED_COLUMNS if c not in df.columns]
if missing:
    raise RuntimeError(f"Dataset schema invalid. Missing columns: {missing}")
...
print("\n============================================================")
print("⚖️  ZOKASCORE RECONCILIATION AUDIT V4 (Strict >10% Edge)")
print("============================================================")
...
ledger.to_csv('public_data/audit_ledger_v4.csv', index=False)
print("\n[Audit V4] Immutable bet ledger saved to public_data/audit_ledger_v4.csv")