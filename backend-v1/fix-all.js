...
const dir = './public_data/fixtures';
...
for(const file of files){
  ...
  fs.writeFileSync(full, JSON.stringify({ data: fixed, count: fixed.length, date: raw.date || file.replace('.json','') }, null, 2));
  console.log('Fixed', file, '->', fixed.length);
}
