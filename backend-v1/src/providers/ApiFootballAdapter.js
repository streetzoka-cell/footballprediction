...
class ApiFootballAdapter extends BaseProvider {
  constructor() {
    ...
    // ★ Multi-key support: 2 keys × 100 calls each = 200 total
    this.keys = [];

    if (env.API_FOOTBALL_KEY) {
      this.keys.push({
        key: env.API_FOOTBALL_KEY,
        remaining: env.API_FOOTBALL_DAILY_BUDGET,
    ...
    // ★ Confirms 2 keys × 100 = 200 total at boot
    logger.info(
      `[ApiFootball] Initialized ${this.keys.length} key(s) × ${env.API_FOOTBALL_DAILY_BUDGET}/key = ` +
      `${this.keys.length * env.API_FOOTBALL_DAILY_BUDGET} total daily budget.`
    );
  }
  ...
  _resetIfNewDay() {
    ...
    this.keys.forEach((k) => {
      if (k.lastResetDate !== today) {
        k.remaining = env.API_FOOTBALL_DAILY_BUDGET;
    ...
    if (reset) {
      logger.info(
        `[ApiFootball] New day - Local budget counter reset for ${this.keys.length} keys.`
      );
    }
  }
  ...
  _setupInterceptors() {
    this.api.interceptors.request.use((cfg) => {
      ...
      if (!activeKey) {
        const err = new Error(
          'All API-Football keys exhausted (0 remaining). Blocked: ' + cfg.url
        );
      ...
      (err) => {
        if (err.response?.status === 429) {
          ...
          if (keyObj) {
            keyObj.remaining = 0;
            logger.warn(
              `[ApiFootball] Key ending in ...${usedKey.slice(-4)} hit 429. Marking as exhausted.`
            );
          }
        }
  ...
  async _fetch(endpoint, params = {}) {
    if (!this.isBudgetAvailable(1)) {
      throw new Error('All API-Football keys exhausted');
    }
  ...
  async getFixtures(dateStr) {
    ...
    logger.info(
      `[ApiFootball] Fetched ${data.length} fixtures for ${dateStr}. Total Remaining: ${this.getRemaining()}`
    );
  ...
  async getLiveFixtures() {
    ...
    logger.info(
      `[ApiFootball] Fetched ${data.length} live matches. Total Remaining: ${this.getRemaining()}`
    );
    return data.map(Normaliser.normalizeMatch);
  }
