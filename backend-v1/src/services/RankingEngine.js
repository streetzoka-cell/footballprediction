...
async function updateZokaPicksForMatch(date, matchId, homeScore, awayScore) {
  try {
    ...
    await db.collection('zoka_picks').doc(String(date)).set(payload, { merge: true });
    ...
    logger.info(`[RankingEngine] Updated Zoka Picks for match ${matchId} on ${date}.`);
    return true;
  } catch (err) {
    logger.warn(`[RankingEngine] Failed to update Zoka Picks for ${matchId}: ${err.message}`);
    return false;
  }
}
 ...
 * - Idempotent: skips users already in prediction_results
 * - Chunked batch writes
 * - Auto-rebuilds leaderboards
 */
async function resolveMatch(input = {}) {
  ...
  try {
    ...
    predsSnap.forEach((d) => {
      ...
      ops.push({ p, uid, points: r.points, resultType: r.resultType });
    });
    ...
    for (let i = 0; i < ops.length; i += OPS_PER_BATCH) {
      ...
      for (const op of chunk) {
        const resultRef = db.collection('prediction_results').doc(`${op.uid}_${matchId}`);
        batch.set(resultRef, {
          userId: op.uid,
        ...
        batch.set(userPointRef, {
          ...
          exactCount: admin.firestore.FieldValue.increment(op.resultType === 'exact' ? 1 : 0),
          resultCount: admin.firestore.FieldValue.increment(op.resultType === 'result' ? 1 : 0),
          missCount: admin.firestore.FieldValue.increment(op.resultType === 'miss' ? 1 : 0),
          predictionsCount: admin.firestore.FieldValue.increment(1),
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });

        const dailyRef = db.collection('daily_leaderboard').doc(matchDate).collection('users').doc(op.uid);
        batch.set(dailyRef, {
          uid: op.uid,
          displayName: op.p.displayName || 'Player',
          points: admin.firestore.FieldValue.increment(op.points),
          exact: admin.firestore.FieldValue.increment(op.resultType === 'exact' ? 1 : 0),
          result: admin.firestore.FieldValue.increment(op.resultType === 'result' ? 1 : 0),
          miss: admin.firestore.FieldValue.increment(op.resultType === 'miss' ? 1 : 0),
      ...
      await Promise.race([
        batch.commit(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Ranking batch timeout')), WRITE_TIMEOUT_MS)),
      ]);
    ...
    // 7. Rebuild Daily Leaderboard Snapshot
    await LeaderboardEngine.publishDailyLeaderboardSnapshot(matchDate).catch(err => {
      logger.warn(`[RankingEngine] Daily leaderboard publish failed for ${matchDate}: ${err.message}`);
    });

    // 8. Rebuild GOAT and Period summaries
    await LeaderboardEngine.rebuildGoat().catch(err => {
      logger.warn(`[RankingEngine] GOAT rebuild failed: ${err.message}`);
    });
    await LeaderboardEngine.rebuildPeriod('weekly').catch(err => {
      logger.warn(`[RankingEngine] Weekly rebuild failed: ${err.message}`);
    });
    await LeaderboardEngine.rebuildPeriod('monthly').catch(err => {
      logger.warn(`[RankingEngine] Monthly rebuild failed: ${err.message}`);
    });

    logger.info(`[RankingEngine] Resolved match ${matchId} for ${matchDate}. Applied: ${applied}. Skipped: ${processed.size}.`);

    return { resolved: true, matchId, matchDate, users: applied, skipped: processed.size };
  } finally {
