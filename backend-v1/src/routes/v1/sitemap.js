...
const readJsonFiles = (directory) => {
  ...
  for (const file of files) {
    try {
      ...
      data.push(...records);
    } catch (error) { console.error(`[Sitemap] Failed to read ${file}:`, error.message); }
  }
  return data;
};
...
// ★ SEO FIX: Added Image Tags for Google Images Indexing
...
const ensureSitemapCache = () => {
  if (Date.now() - sitemapCache.lastUpdated > CACHE_TTL || !sitemapCache.static) rebuildSitemapCache();
};
...
router.get(["/", "/sitemap.xml"], (req, res) => {
  ...
  } catch (error) { res.status(500).send("Sitemap generation failed."); }
});
