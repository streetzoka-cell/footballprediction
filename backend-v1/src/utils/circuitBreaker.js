...
async function trip(name, reason = 'unknown') {
  if (!breakers[name]) {
    breakers[name] = { failures: 0, tripped: false, resetTime: 0 };
  }
  ...
  if (breakers[name].failures >= 3 && !breakers[name].tripped) {
    ...
    breakers[name].resetTime = Date.now() + 5 * 60 * 1000;
    logger.warn(
      `[CircuitBreaker] Tripped: ${name} (Reason: ${reason}). Disabled for 5 mins.`
    );
  }
}
