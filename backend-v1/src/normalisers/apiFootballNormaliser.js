...
const normalizeMatch = (m) => {
  ...
  const normalizedData = {
    ...
    score: {
      ...
      extratime: {
        home: m.score?.extratime?.home ?? null,
        away: m.score?.extratime?.away ?? null,
      },
      penalty: {
        home: m.score?.penalty?.home ?? null,
        away: m.score?.penalty?.away ?? null,
      },
    },
...
module.exports = {
  normalizeMatch,
  matches: (payload) => (payload || []).map(normalizeMatch).filter(Boolean),
};