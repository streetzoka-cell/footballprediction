// backend-v2/src/config/firebase.js

const admin = require('firebase-admin');
const env = require('./env');
const logger = require('../utils/logger');
const { BATCH_MAX_OPS, WRITE_TIMEOUT_MS, COLLECTIONS } = require('./constants');
...
async function deleteByIds(collectionPath, ids) {
  ...
  for (let i = 0; i < ids.length; i += BATCH_MAX_OPS) {
    ...
    await Promise.race([
      batch.commit(),
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Delete timeout')), WRITE_TIMEOUT_MS)
      ),
    ]);
...
async function clearCollection(collectionPath) {
  ...
  const colRef = database.collection(collectionPath);
  let totalDeleted = 0;
  let query = colRef.limit(BATCH_MAX_OPS * 2);
