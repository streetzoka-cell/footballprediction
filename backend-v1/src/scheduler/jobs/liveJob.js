...
async function execute() {
  ...
  try {
    ...
    if (!QuotaManager.canPollLive()) {
      ...
      logger.warn(
        '[LiveJob] Live polling blocked. Daily budget exhausted.'
      );
    ...
    if (result.skipped) {
      ...
      logger.info(
        '[LiveJob] Sync skipped by service cache'
      );
    ...
    /*
      ...
      When a match disappears from live feed,
      refresh FT once.
    */
    if (prevLiveIds.size > 0) {
      ...
      if (
        ...
        QuotaManager.canFetchFT()
      ) {


        logger.info(
          `[LiveJob] ${finishedIds.length} match(es) finished. Fetching FT results.`
        );
    ...
    */
    ...
    const stats =
      QuotaManager.getStats();



    logger.info(
      `[LiveJob] Next poll ${intervalMs / 60000}m | ` +
      `Live ${currentLiveCount} | ` +
      `Live remaining ${stats.liveRemaining} | ` +
      `FT remaining ${stats.ftRemaining}`
    );
