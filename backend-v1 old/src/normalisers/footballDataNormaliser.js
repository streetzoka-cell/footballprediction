// footballprediction/backend-v1/src/normalisers/footballDataNormaliser.js

const normalizeStandings = (data) => {
  if (!data || !data.standings || data.standings.length === 0) return null;
  
  // We only want the "TOTAL" standings (type: "TOTAL")
  ...
  const standingsTable = totalStandings.table.map(row => ({
    rank: row.position,
    team: { id: String(row.team.id), name: row.team.name, logo: row.team.crest },
    points: row.points,
  ...
  return {
    ...
    logo: null, // football-data.org doesn't provide league logos
    season: data.season.startDate.split('-')[0],
    standings: [standingsTable] // Wrap in array to match our internal format
  };
};
