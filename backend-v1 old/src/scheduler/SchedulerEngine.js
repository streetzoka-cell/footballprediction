// footballprediction/backend-v1/src/scheduler/SchedulerEngine.js
...
class SchedulerEngine {
  ...
  async _runJob(name, taskFn) {
    ...
    try {
      logger.info(`[SchedulerEngine] â†’ Running: ${name}`);
      const result = await taskFn();
      const dur = Date.now() - start;
      metrics.record(name, true, dur);
      logger.info(`[SchedulerEngine] âœ“ ${name} completed in ${dur}ms`);
      return result;
    } catch (err) {
      const dur = Date.now() - start;
      metrics.record(name, false, dur);
      logger.error(`[SchedulerEngine] âœ— ${name} failed: ${err.message}`);
      throw err;
    }
  }
