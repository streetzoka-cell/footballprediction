// footballprediction/backend-v1/src/scheduler/index.js
...
const internetMonitor = require('../services/InternetMonitor'); // â˜… NEW
...
function startScheduler() {
  ...
  schedulerEngine.startLivePolling(async () => {
    // â˜… Pause live polling if offline
  ...
  // â˜… AUTO CATCH-UP: The second internet returns, run all jobs immediately!
  internetMonitor.on('restored', async () => {
    logger.info('[Scheduler] ðŸš€ Catch-up sync triggered...');
    try {
      await todayFixturesJob.execute();
      await finishedFixturesJob.execute(true);
      await liveJob.execute();
